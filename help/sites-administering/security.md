...
通过选中或清除各个AEM [操作](security.md#actions)的复选框，可以更改授予/拒绝给给定用户的权限。 复选标记表示允许执行某个操作。 无复选标记表示操作被拒绝。
...
>
>在创建特定于社区的ACL时，加入这些社区的成员可能会被授予附加权限。 例如，当用户加入位于`/content/we-retail/us/en/community`的社区时
...
在修改任一权限之前，请确保您了解它们的工作方式和相互关系。 请参阅说明AEM WCM [如何评估访问权限](/help/sites-administering/user-group-ac-admin.md#how-access-rights-are-evaluated)的CRX文档，以及有关设置访问控制列表的示例。
...
1. 在&#x200B;**安全性**&#x200B;控制台树列表中，单击&#x200B;**编辑**，然后单击&#x200B;**创建用户**&#x200B;或&#x200B;**创建组**。
