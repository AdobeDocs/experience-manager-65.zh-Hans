---
title: AEM 6.5 管理用户指南
description: 了解如何管理 AEM 6.5。这些指南涉及网站功能、部署操作、安全功能、个性化、集成等内容。
exl-id: 299c783a-1495-4c0d-8431-cd7de142a163
solution: Experience Manager, Experience Manager Sites
feature: Administering
role: Admin
source-git-commit: 48d12388d4707e61117116ca7eb533cea8c7ef34
workflow-type: tm+mt
source-wordcount: '0'
ht-degree: 0%

---


# AEM 6.5 管理用户指南

了解如何管理 AEM 6.5。这些指南涉及网站功能、部署操作、安全功能、个性化、集成等内容。

## 关键管理文章

* [多网站管理](/help/sites-administering/msm.md)
* [MSM 和翻译](/help/sites-administering/msm-and-translation.md)
* [操作仪表板](/help/sites-administering/operations-dashboard.md)
* [备份和恢复](/help/sites-administering/backup-and-restore.md)
* [个性化](/help/sites-administering/personalization.md)
* [第三方集成](/help/sites-administering/third-party-services.md)
