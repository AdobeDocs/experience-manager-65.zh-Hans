...
* 子 `property` 元素为所有资源类型标识要翻译的节点属性：
   ...
   * `name` 属性包含属性名。
   * 可选 `translate` 属性等于 `false`（如果该属性未翻译）。 默认情况下，值为`true`。 在覆盖以前的规则时，此属性很有用。
...
以下示例将翻译所有`text`属性的内容以及foundation图像组件的其他属性。 如果其他组件具有同名属性，则该规则不适用于它们。
...
**isDeep**&#x200B;此属性适用于节点筛选器，默认情况下为true。 它检查节点（或其祖先）是否在过滤器中包含具有指定属性值的属性。 如果为 false，则仅检查当前节点。

例如，即使父节点将属性 `draftOnly` 设置为 true 以标记草稿内容，子节点也会添加到翻译作业中。 `isDeep`检查父节点是否将属性`draftOnly`设置为true并排除这些子节点。
...
**inherit**&#x200B;这适用于属性。 默认情况下，每个属性都会被继承，但如果您希望某个属性不被子级继承，则可以将该属性标记为false，使其仅应用于该特定节点。
...
**updateDestinationLanguage**&#x200B;此属性用于没有文本但有语言代码的属性，如`jcr:language`。 用户不会翻译文本，而是进行从源到目标的语言区域设置。 不会发送此类属性进行翻译。
