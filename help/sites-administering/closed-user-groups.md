...
在对CUG策略进行的新的AEM默认设置评估中，仅在“发布”运行模式下启用它。 有关更多详细信息，请参阅AEM 6.3[&#128279;](#default-configuration-since-aem)之后的默认配置的详细信息。 这可以通过比较给定路径的有效策略与内容中存储的策略来验证。 只有启用CUG的权限评估后，才会显示有效的策略。
