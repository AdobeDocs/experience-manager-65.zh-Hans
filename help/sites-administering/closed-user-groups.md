...
CUG策略的权限评估以及与默认授权模型或任何其他授权模型的交互遵循为Apache Jackrabbit Oak中的多个授权机制设计的模式。 也就是说，当且仅当所有模型都授予访问权限时，才授予给定权限集。 请参阅 [此页面](https://jackrabbit.apache.org/oak/docs/security/authorization/composite.html) 以了解更多详细信息。
...
CUG功能的与身份验证相关的部分允许您标记需要身份验证的树，并可以选择指定专用登录页面。 根据以前的版本，新的实施允许您在内容存储库中标记需要身份验证的树。 它还可以有条件地实现与 `Sling org.apache.sling.api.auth.Authenticator`负责最终强制实施该要求并重定向到登录资源。

这些要求由OSGi服务向Authenticator注册，该服务提供 `sling.auth.requirements` 注册属性。 然后，使用这些属性来动态扩展身份验证要求。 欲知更多详情，请参见 [Sling文档](https://sling.apache.org/apidocs/sling7/org/apache/sling/auth/core/AuthConstants.html#AUTH_REQUIREMENTS).
...
这同样适用于 `granite:loginPath` 属性。 只有当它由与身份验证要求相关的mixin类型定义时，才会遵守它。 在非结构化JCR节点中添加具有此名称的剩余属性不会显示所需的效果，并且负责更新OSGi注册的处理程序会忽略该属性。
...
添加 `granite:AuthenticationRequired` 配置的支持路径中的mixin类型导致更新负责处理程序的OSGi注册，其中包含新的、额外的条目 `sling.auth.requirements` 属性。 如果给定的身份验证要求指定了可选 `granite:loginPath` 属性，该值还向验证器注册，前缀为“ — ”，以从验证要求中排除。
...
评估登录路径并在验证时重定向到相应资源是AdobeGranite登录选择器身份验证处理程序( `com.day.cq.auth.impl.LoginSelectorHandler`)，默认情况下是使用AEM配置的Apache Sling AuthenticationHandler。

调用时 `AuthenticationHandler.requestCredentials` 此处理程序尝试确定用户被重定向到的映射登录页面。 这包括以下步骤：
...
此 `LoginPathProvider` 由Granite中新的身份验证要求支持实施的操作会公开由定义的登录路径 `granite:loginPath` 属性，这些属性又由mixin类型定义，如上所述。 保存登录路径的资源路径与属性值本身的映射将保存在内存中，并且被评估为层次结构中的其他节点找到合适的登录路径。
...
Oak文档涵盖了新的CUG策略在存储库内容中的反映方式。 欲知更多信息，请查阅 [此页面](https://jackrabbit.apache.org/oak/docs/security/authorization/cug.html#Representation_in_the_Repository).
...
使用JCR访问控制管理API管理用于限制CUG读取访问的新类型访问控制策略，并遵循中所述的机制。 [JCR 2.0规范](https://developer.adobe.com/experience-manager/reference-materials/spec/jcr/2.0/16_Access_Control_Management.html).

#### 设置新的CUG策略 {#set-a-new-cug-policy}

此代码用于在之前未设置CUG的节点上应用新的CUG策略。 请注意 `getApplicablePolicies` 仅返回以前未设置的新策略。 最后，必须回写策略并保留更改。
...
编辑现有CUG策略需要执行以下步骤。 修改后的策略必须写回，更改必须保留使用 `javax.jcr.Session.save()`.
...
创建包含登录路径的身份验证要求的步骤。 仅当满足以下条件时，才会向Apache Sling Authenticator注册登录路径的要求和排除： `RequirementHandler` 已为包含目标节点的树进行了配置。
...
下面详细说明了更改现有登录路径的步骤。 仅当出现以下情况时，该修改才会向Apache Sling身份验证程序注册： `RequirementHandler` 已为包含目标节点的树进行了配置。 先前的登录路径值将从注册中删除。 此修改不会影响与目标节点关联的身份验证要求。
...
您还必须配置 [Sling引用过滤器](/help/sites-administering/security-checklist.md#the-sling-referrer-filter) 所有可用于访问AEM的主机名；例如，通过CDN、负载平衡器和任何其他。
...
在前一实施中，已采用免除个人主参与者的CUG评估。 新的CUG授权通过名为CugExclude的专用接口覆盖这一点。 Apache Jackrabbit Oak 1.4附带默认实施，该实施不包括固定主体集和允许您配置各个主体名称的扩展实施。 后者在AEM发布实例中配置。
...
|---|---|
| 支持的路径  `/content` | 在存储库中定义的身份验证要求，由 `granite:AuthenticationRequired` mixin类型在下方生效 `/content` 于 `Session.save()`. Sling身份验证器已更新。 在支持的路径之外添加mixin类型会被忽略。 |
...
要禁用对身份验证要求的支持，请按照 `granite.auth.authhandler` 模块，只需删除与关联的配置即可 **AdobeGranite身份验证要求和登录路径处理程序**.
...
在新实施中，默认授权模型的访问控制设置不受任何正在创建、修改或删除的CUG的影响。 而是使用一种名为的新策略类型 `PrincipalSetPolicy` 作为目标节点的附加访问控制内容应用。 此附加策略位于目标节点的子节点中，如果存在，则将成为默认策略节点的同级。
...
换言之，对于有效权限的评估，既有 `CUGPolicy` 并且考虑默认访问控制条目，并且仅当这两种策略都授予CUG内容时，才会授予对该内容的读取访问权限。 在默认AEM发布安装中，对完整版的读取权限 `/content` 树被授予每个人，CUG政策的效果与旧实施相同。
...
在以前的实现中，CUG的授权和身份验证两个层面都是由一个JCR属性( `cq:cugEnabled`)。 就身份验证而言，这导致更新了随Apache Sling Authenticator实施一起存储的身份验证要求列表。 在新实施中，使用专用的mixin类型( `granite:AuthenticationRequired`)。

#### 用于排除登录路径的属性 {#property-for-excluding-login-path}

mixin类型定义一个名为的可选属性 `granite:loginPath`，基本上对应于 `cq:cugLoginPage` 属性。 与以前的实现不同，登录路径属性仅在其声明节点类型是上面提到的mixin时才被考虑。 添加具有该名称的属性而不设置mixin类型不会产生任何效果，并且不会向验证者报告对登录路径的新要求或排除项。
