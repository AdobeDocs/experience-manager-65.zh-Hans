...
![客户端上下文窗口的示例](assets/clientcontext_alisonparker.png)
...
![客户端上下文窗口的“编辑”、“加载”和“重置”图标](do-not-localize/clientcontext_icons.png)

* **编辑**
将打开一个新页面，允许您编辑、添加或删除配置文件属性[&#128279;](#editingprofiledetails)。

* **加载**
您可以[从配置文件列表中进行选择并加载要测试的配置文件](#loading-a-new-user-profile)。

* **重置**
您可以[将配置文件](#resetting-the-profile-to-the-current-user)重置为当前用户的配置文件。

## 可用的客户端上下文组件 {#available-client-context-components}

Client Context可以显示以下属性（[取决于使用“编辑”](#adding-a-property-component)选择的内容）：
...
**活动流**&#x200B;此流提供有关用户在各种平台上的社交活动的信息；例如，AEM论坛、博客、评级等。
...
**通用存储属性**&#x200B;是显示存储内容的通用组件。 它是通用存储组件的较高级版本。

通用存储属性组件包括一个默认呈现器，该呈现器列出了配置的属性（以及缩略图）。

**地理位置**&#x200B;显示客户端的纬度和经度。 它使用HTML5地理位置API在浏览器中查询当前位置。 这会导致向访客显示弹出窗口，浏览器会询问访客是否同意共享其位置。

在Context Cloud中显示时，该组件使用Google API将地图显示为缩略图。 该组件受Google API [使用限制](https://developers.google.com/maps/documentation/staticmaps/intro#Limits)的约束。

>[!NOTE]
>
>在AEM 6.1中，地理位置存储不再提供反向地理编码功能。 因此，地理位置存储不再检索有关当前位置的详细信息，如城市名称或国家/地区代码。 使用此存储数据的区段将无法正常运行。 地理位置存储仅包含位置的纬度和经度。

**JSONP Store**&#x200B;显示依赖您安装的内容的组件。

JSONP标准是JSON的补充，允许规避相同源策略（使得Web应用程序无法与位于其他域上的服务器通信）。 它包含在函数调用中封装JSON对象，以便能够将其作为`<script>`从其他域加载（这是允许对同一源策略执行的异常）。

JSONP存储与任何其他存储一样，但它可以加载来自其他域的信息，而无需在当前域上为该信息设置代理。 请参阅[通过JSONP将数据存储在客户端上下文中的示例](/help/sites-administering/client-context.md#storing-data-in-client-context-via-jsonp)。
...
**已解析的段**&#x200B;显示当前解析的段（通常取决于客户端上下文中显示的其他信息）。 这在配置营销活动时很有用。
...
>
>`/home/users/geometrixx/aparker@geometrixx.info/profile` =>朋友属性

**标记云**&#x200B;显示在当前页面上设置的标记以及在浏览网站时收集的标记。 将鼠标悬停在标签上会显示当前用户访问包含该特定标签的页面的次数。

>[!NOTE]
>
>在DAM资产上设置的标记（显示在访问的页面中）不会被计数。

**Technographics存储**&#x200B;此组件依赖于您的安装。

**ViewedProducts**&#x200B;跟踪购物者已查看的产品。 可查询最近查看过的产品或购物车中尚不存在的最近查看过的产品。

此会话存储没有默认的客户端上下文组件。

有关详细信息，请参阅[客户端上下文详细信息](/help/sites-developing/client-context.md)。
...
1. 单击Load Profile图标：
   ...
   ![客户端上下文的加载配置文件图标](do-not-localize/clientcontext_loadprofile.png)
...
编辑客户端上下文可用于设置（或重置）某些属性的值。 这允许您测试特定方案（对[分段](/help/sites-administering/campaign-segmentation.md)和[促销活动](/help/sites-classic-ui-authoring/classic-personalization-campaigns.md)特别有用）。

![编辑客户端上下文](assets/clientcontext_alisonparker_edit.png)

### 添加属性组件 {#adding-a-property-component}

打开&#x200B;**ClientContext设计页**&#x200B;后，您还可以使用可用组件&#x200B;**添加**&#x200B;全新属性（组件在Sidekick上或双击&#x200B;**将组件或资产拖动到此处**&#x200B;框后打开的&#x200B;**插入新组件**&#x200B;对话框中列出）：

![将属性添加到客户端上下文窗口](assets/clientcontext_alisonparker_new.png)

### 删除属性组件 {#removing-a-property-component}

打开&#x200B;**ClientContext设计页**&#x200B;后，如果不再需要，您还可以&#x200B;**删除**&#x200B;属性。 这包括提供的现成属性；**重置**&#x200B;将恢复这些属性（如果已删除）。
...
此示例使用Geometrixx Outdoors示例网站来访问Client Context并测试创建的区段。 只要该页面启用了Client Context，您就可以使用其他网站。 （请参阅[将客户端上下文添加到页面](/help/sites-developing/client-context.md#adding-client-context-to-a-page)。）
...
1. 在Web浏览器(`https://localhost:4502/miscadmin#/etc`)中打开“工具”控制台。
...
1. 将常规存储属性拖到默认的AND容器中。
   ...
   ![向区段编辑器添加组件](assets/chlimage_1-5.jpeg)
