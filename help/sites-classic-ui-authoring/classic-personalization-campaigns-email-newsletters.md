...
此 **电子邮件工具** exactTarget的组件可以向您的电子邮件/新闻稿添加更多电子邮件功能。
...
<table>
 <tbody>
  ...
  <tr>
   <td>跟踪电子邮件打开次数</td>
   <td>一个隐藏组件，允许您使用ExactTarget跟踪功能。<br /> </td>
  </tr>
 </tbody>
</table>
...
>[!NOTE]
>
>此 **选项** 仅当将ExactTarget配置应用于电子邮件时，才会填充下拉菜单。 请参阅 [将电子邮件服务配置应用到电子邮件设置](#applying-e-mail-service-configuration-to-e-mail-settings) 以了解更多信息。
...
>[!NOTE]
>
>* 仅当使用发送电子邮件时，电子邮件工具中的URL才会被替换为实际值 **简单发送** 或 **引导式发送** 但不是 **测试发送**.
>
>* 需要以下两种电子邮件工具： **邮寄地址（必填）** 和 **配置文件中心（必需）**. 将电子邮件发布到ExactTarget后，默认情况下会将这两种电子邮件工具添加到每封邮件的底部。
>
...
1. 打开要发布到电子邮件服务的电子邮件。
1. 要从电子邮件服务启用个性化字段，请在配置电子邮件服务时添加框架配置。 请参阅 [配置Silverpop Engage](/help/sites-administering/silverpop.md) 和 [配置Exact Target](/help/sites-administering/exacttarget.md) 以了解更多信息。
1. 添加组件 **文本和个性化** 从副手手手手里。 此组件是新闻稿组的一部分。 在编辑模式下打开此组件。

   ![chlimage_1-110](assets/chlimage_1-110a.png)

1. 通过选择下拉菜单中的字段并单击，将所需的个性化字段添加到文本 **插入**.
1. 单击 **确定** 完成。
...
1. 单击 **添加服务** 在 **Cloud Service** 选项卡。 您会看到服务列表。 选择所需的配置 —  **ExactTarget** 或 **Silverpop**  — 从下拉列表中的列表。
...
1. 单击&#x200B;**发布**。
   ...
   如果存在ExactTarget，则通过单击可查看已发布的电子邮件 **查看发布的电子邮件**. 这会将您直接转到ExactTarget中已发布的新闻稿([https://members.exacttarget.com/](https://members.exacttarget.com/).)。
