...
中介绍了如何准备设计包以进行导入 [扩展和配置设计导入程序](/help/sites-administering/extending-the-design-importer-for-landingpages.md). 中介绍了与Adobe Analytics集成 [集成Landing Pages与Adobe Analytics。](/help/sites-administering/integrating-landing-pages-with-adobe-analytics.md)
...
>[!CAUTION]
>
>因为设计导入程序需要访问 `/apps`，它无法在容器化云环境中运行，其中 `/apps` 是不可变的。
...
登陆页面也可以具有页面的移动设备版本。 要获得单独的登陆页面移动版本，导入设计必须具有两个html文件： *index.htm(l)* 和 *mobile.index.htm(l)*.

登陆页面的导入过程与普通登陆页面的导入过程相同，登陆页面设计具有一个与移动设备登陆页面对应的附加html文件。 此html文件也必须具有画布 `div` 替换为 `id=cqcanvas` 与桌面登陆页面html一样，该页面支持针对桌面登陆页面描述的所有可编辑组件。
...
文本组件允许您使用WYSIWYG编辑器输入文本块。 请参阅 [文本组件](/help/sites-authoring/default-components.md#text) 以了解更多信息。
...
利用标题组件，可显示标题并配置大小(h1-6)。 请参阅 [标题组件](/help/sites-authoring/default-components.md#title) 以了解更多信息。
...
图像组件显示一个图像，您可以从内容查找器拖放或单击该图像以上传。 请参阅 [图像组件](/help/sites-authoring/default-components.md) 以了解更多信息。
...
此CTA组件可用于添加登陆页面上带有链接的任何图形图像。 图像可以是简单的按钮，也可以是作为背景的任何图形图像。 单击图像时，用户将被带入组件属性中指定的目标URL。 它是 **行动动员** 组。
...
前文显示了一个示例表单；它由 **潜在客户表单** 组件（开始和结束），带有 **名字** 和 **电子邮件ID** 用于输入和 **提交** 字段
...
>您看到的字段因各个组件而异。
>
>并非所有选项都可用于所有潜在客户表单组件。 请参阅Forms以了解有关这些方面的更多信息 [通用设置](/help/sites-authoring/default-components.md#formsgroup).
...
Lead Form组件使用 **表单开始** 和 **表单结尾** 元素。 它们始终是成对的，以确保正确定义表单。

添加潜在客户表单后，您可以通过单击来配置表单的开头或结尾 **编辑** 在相应的栏中。
...
**操作配置** 根据“Post Options（发布选项）”中的选择，操作配置会发生变化。 例如，当您选择“创建潜在客户”时，您可以配置要将潜在客户添加到哪个列表中。
...
由于页面导入涉及页面的HTML准备、验证和测试，因此登陆页面的导入是一项管理员任务。 作为管理员，执行导入的用户需要对拥有读取、写入、创建和删除权限 `/apps`. 如果用户没有这些权限，导入将失败。
...
1. 在左窗格中选择您的营销策划。
1. 单击 **新建** 以打开 **创建页面** 窗口。
1. 选择 **导入程序页面** 模板并添加标题和（可选）名称，然后单击 **创建**.
...
1. 打开导入器页面，您 [先前创建](#creatingablankcanvaspage).
...
导入设计包时， **清除设计** 和 **下载导入的Zip** 在页面的设置菜单中提供：
...
如果您要在对登陆页面设计包进行某些更改后重新导入该设计包，则可以通过单击 **清除** ，或者单击触控优化用户界面中设置菜单中的清除。 这样做会删除导入的登陆页面并创建一个空白的导入程序页面。

清除登陆页面时，您可以删除内容更改。 如果您单击 **否**，则会保留内容更改，即下的结构 `jcr:content/importer`将保留，并且仅保留中的导入程序页面组件和资源 `etc/design` 将被删除。 但是，如果您单击 **是**， `jcr:content/importer` 也会被删除。
