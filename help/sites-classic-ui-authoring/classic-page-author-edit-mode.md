...
有些可以通过sidekick立即获取，而其他各种也可通过使用 [设计模式](/help/sites-classic-ui-authoring/classic-page-author-design-mode.md) 以启用/禁用它们。
...
组件在以下情况下可用 [编辑页面](/help/sites-classic-ui-authoring/classic-page-author-edit-content.md) 从 **组件** 副手和副手的标签 **插入新组件** 选择器(当您双击 **将组件或资源拖动到此处** 区域)。
...
您可以从以下位置拖放图像 [内容查找器](/help/sites-classic-ui-authoring/classic-page-author-env-tools.md#the-content-finder) 直接转到组件或其“编辑”对话框。 您还可以双击“编辑”对话框的中心区域来浏览本地文件系统并上传图像。 “编辑”对话框的两个选项卡还控制图像的所有定义和操作：
...
以下示例显示了 **列表** 以某种方式显示子页面列表的组件（设计由网站设计的自定义CSS定义控制）。
...
* 视频
   ...
   * **大小**
     ...
     视频的本机大小（宽度x高度，以像素为单位）将显示在“大小”旁边的框中（请参阅上文）。 如果要覆盖视频的本机尺寸，请在此处手动输入宽度和高度尺寸。 选择 **确定** 关闭对话框。
...
表单组件使用定义表单的开头和结尾 **表单开始** 和 **表单结尾** 元素。 开始元素和结束元素始终是配对的，以确保正确定义表单。
