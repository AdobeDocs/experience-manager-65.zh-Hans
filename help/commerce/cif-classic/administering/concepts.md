...
为了优化运营，AEM和电子商务引擎都专注于各自的专业领域。 信息在两者之间实时传输；例如：
...
将数据从商业引擎导入到AEM电子商务网站时，会使用商业提供商向导入者提供数据。 一个商务提供商可以支持多个导入程序。

商业提供商是自定义到的AEM代码，可用于：
...
如果可以区分以下两个类别，那么通过这可以清楚地显示具有有意义的结构（包含`cq:Page`个节点的树）的URL，因此非常接近于经典AEM内容管理：
...
无论哪种情况，当产品数据已输入/导入AEM时，都可以从&#x200B;**Products**&#x200B;控制台中看到该数据。 在此处，产品的卡和列表视图会显示如下信息：
...
每个产品的各个属性可能取决于所使用的电子商务引擎以及AEM实施。 在查看产品页面和/或编辑产品信息时，这些选项可用（如果适用），可以包括：
...
标记也可用于将产品分组到目录中。 它们可用于更具动态的目录，例如季节性优惠。
...
从电子商务引擎(PIM)导入包含大量产品（超过100,000件）的大型目录可能会由于节点过多而影响系统。 如果产品具有关联的资产（例如，产品图像），这也会减慢创作实例的速度。 这是因为这些资源的后处理需要占用大量的CPU和内存。
...
![电子商务类别正在运行](/help/sites-administering/assets/ecommerce_categoryrunning.png)
...
![电子商务_nairobirunnersgreen](/help/sites-administering/assets/ecommerce_nairobirunnersgreen.png)
...
这些主要是提供静态信息的AEM页面；例如，带有基础产品页面链接的简介和概述。

### 产品组件 {#product-component}

可以将&#x200B;**Product**&#x200B;组件添加到任何具有提供所需元数据的父页面（即`cartPage`和`cartObject`的路径）的页面。 在演示站点Geometrixx Outdoors中，此项由`UserInfo.jsp`提供。
...
1. 该用户帐户仅存在于AEM中：
   ...
   1. 用户在电子商务引擎中创建，并具有相同的帐户ID和将存储在AEM中的随机密码。
   1. 由于AEM会尝试在第一次调用时登录到电子商务引擎（例如，当请求产品页面并且价格引用电子商务引擎时），因此随机密码是必需的。 由于此过程发生在AEM登录之后，因此密码不可用。
...
>
>否则，这些其他AEM实例还将尝试在引擎中为同一用户创建帐户。 这些操作因来自引擎的`DuplicateUidException`而失败。
...
来自电子商务引擎的交易数据与购物者的个人信息相结合。 AEM将这些数据中的一部分用作配置文件数据。 AEM中的表单操作将信息写回电子商务引擎。

有一个页面可让您轻松管理帐户信息。 您可以通过单击Geometrixx页面顶部的&#x200B;**我的帐户**&#x200B;或导航到`/content/geometrixx-outdoors/en/user/account.html`来访问它。

![chlimage_1-13](/help/sites-administering/assets/chlimage_1-13.png)

### 通讯簿 {#address-book}

您的网站需要存储选择的地址；包括投放、帐单和备用地址。 您可以使用基于默认地址格式的表单来实现此目的，也可以使用AEM提供的通讯簿组件。
...
您可以单击&#x200B;**添加新地址……**&#x200B;以在您的通讯簿中添加地址。 它会打开一个您可以填写的表单，然后单击&#x200B;**添加地址**。
...
电子商务引擎使用上下文（主要是购物者信息）来确定其价格，然后将正确信息提供回AEM。
...
* 以`Allison Parker`身份再次登录 — Allison的产品已恢复
...
订单履行和跟踪由电子商务引擎管理。 AEM可以使用订单历史记录组件显示信息，该组件会显示所有相关详细信息，包括应用的优惠券和促销活动。 例如：

![chlimage_1-17](/help/sites-administering/assets/chlimage_1-17.png)

## 签出 {#checkout}

使用标准AEM表单实施结账。 这允许营销经理使用营销内容自定义体验。

然后，电子商务利用AEM表单中的输入来管理结账过程。

### 支付安全性 {#payment-security}

包括信用卡信息在内的付款详细信息通常由电子商务引擎管理。 AEM将此类交易信息转发给引擎（然后从转发给支付处理服务）。
