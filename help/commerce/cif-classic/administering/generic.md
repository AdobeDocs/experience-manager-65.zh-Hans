...
1. 指定：
   ...
   * **导入程序**
     ...
     特定的Importer [商业提供程序](/help/commerce/cif-classic/administering/concepts.md#commerce-providers)，默认情况下 `Geometrixx`.
...
>[!NOTE]
>
>标准产品管理是基本的，因为Geometrixx-Outdoors产品集是基本的。 复杂性取决于产品 [基架](/help/sites-authoring/scaffolding.md)，因此使用您自己的产品基架，可以实现更复杂的编辑。

#### 创建产品信息 — 触屏优化UI {#creating-product-information-touch-optimized-ui}

1. 使用 **产品** 控制台(通过 **商务**)导航到所需的位置。
1. 使用 **创建** 图标以选择任一选项（具体取决于结构和位置）：
...
1. 根据需要修改和移动形状。
1. 单击形状。
1. 单击浏览图标将打开 [资产选取器](/help/assets/search-assets.md#assetpicker).
...
>[!NOTE]
>
>以下示例处理直接在中持有的促销 [营销活动](/help/sites-classic-ui-authoring/classic-personalization-campaigns.md)，这用于优惠券。
>
