...
1. 在Adobe CQ Scene7 ImageServer对话框中，设置以下配置值：
   ...
   | `AllowRemoteAccess.name` | *`empty`* | 允许或禁止远程访问ImageServer进程。 如果为false，则图像服务器仅在localhost上侦听。<br> 指向本地主机的默认外部化程序设置必须指定特定VM实例的实际域或IP地址。 原因是localhost指向VM的父系统。<br>VM的域或IP地址必须具有主机文件条目，以便它能够自行解析。 |
