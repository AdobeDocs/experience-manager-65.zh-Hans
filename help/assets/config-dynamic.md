...
>* `ImageServing.log`
>
>记录于 [监控和维护Experience Manager实例](/help/sites-deploying/monitoring-and-maintaining.md).
...
| “发布”节点Experience Manager | 红色正方形中的“X”白色。 | 您部署到On-Premise或通过Managed Services的发布节点。 |
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>
    <ol>
     ...
     <li>在中配置映像 <a href="#configuring-dynamic-media-cloud-services">Dynamic MediaCloud Service</a>.</li>
  ...
  <tr>
   ...
   <td>图像通过Experience Manager发布节点交付。 在此方案中，由于流量最小，因此无需将图像传送到Adobe的数据中心。 它还允许在生产启动之前安全预览内容。</td>
   <td>
    <ol>
     <li>在Experience Manager上 <strong>作者</strong> 节点， <a href="#enabling-dynamic-media">启用Dynamic Media</a>.</li>
  ...
  <tr>
   ...
   <td>
    <ol>
     <li>在Experience Manager上 <strong>作者</strong> 节点， <a href="#enabling-dynamic-media">启用Dynamic Media</a>.</li>
     <li>在Experience Manager上 <strong>发布</strong> 节点， <a href="#enabling-dynamic-media">启用Dynamic Media</a> （发布实例提供视频海报图像，并提供用于视频播放的元数据）。</li>
     <li>在中配置视频 <a href="#configuring-dynamic-media-cloud-services">Dynamic MediaCloud Service。</a></li>
     <li><a href="#replicating-viewer-presets">复制查看器预设</a>.</li>
  ...
  <tr>
   <td>在生产环境中交付图像和视频</td>
   <td><p>视频通过CDN交付和缓存，以实现可扩展性能和全球覆盖。 图像和视频海报图像通过Adobe全球数据中心的服务器提供，然后通过CDN缓存以实现可扩展性能和全球覆盖。</p> <p>请参阅之前的部分，以在预生产环境中设置图像或视频。 </p> </td>
   <td>
    <ol>
     <li>在Experience Manager上 <strong>作者</strong> 节点， <a href="#enabling-dynamic-media">启用Dynamic Media</a>.</li>
     <li>在中配置视频 <a href="#configuring-dynamic-media-cloud-services">Dynamic MediaCloud Service。</a></li>
...
[Dynamic Media](https://business.adobe.com/products/experience-manager/assets/dynamic-media.html) 默认情况下处于禁用状态。 要利用Dynamic MediaDynamic Media功能，您必须通过使用 `dynamicmedia` 运行模式，例如， `publish` 运行模式。 在启用之前，请务必查阅 [技术要求](/help/sites-deploying/technical-requirements.md#requirements-for-aem-dynamic-media-add-on).

>[!NOTE]
>
>通过运行模式启用Dynamic Media将取代Experience Manager6.1和Experience ManagerDynamic Media 6.0中的功能，在这些功能中，您通过设置 `dynamicMediaEnabled` 标记到 **[!UICONTROL true]**. 此标记在Experience Manager6.2及更高版本中没有功能。 此外，您无需重新启动快速入门即可启用Dynamic Media。

启用Dynamic Media后，Dynamic Media功能在UI中可用，并且每个上传的图像资产都会收到 *cqdam.pyramid.tiff* 用于快速投放动态图像演绎版的演绎版。 这些PTIFF具有下列显着优势：

* 仅管理单个主源映像并动态生成无限演绎版的能力，无需任何额外存储。
* 能够使用交互式可视化图表，例如缩放、平移和旋转。

如果要在Experience Manager中使用Dynamic Media Classic，请勿启用Dynamic Media，除非您使用 [特定方案](/help/sites-administering/scene7.md#aem-scene-integration-versus-dynamic-media). 除非通过运行模式启用Dynamic Media，否则将禁用Dynamic Media。
...
1. 在命令行上，启动快速启动时，执行以下操作：
   ...
   * 添加 `-r dynamicmedia` 启动jar文件时命令行末尾。
...
1. 请求 `https://localhost:4502/is/image` 并确保图像服务器正在运行。
   ...
   * s7access-&lt;yyyy>&lt;mm>&lt;dd>.log - s7access日志记录通过向Dynamic Media发出的每个请求 `/is/image` 和 `/is/content`.
   >
   这些日志仅在启用Dynamic Media时使用。 它们未包含在 **下载完整部分** 从中生成的包 `system/console/status-Bundlelist` 页面；如果您遇到Dynamic Media问题，在致电客户支持时，将这两个日志附加到问题中。

### 如果您将Experience Manager安装到其他端口或上下文路径…… {#if-you-installed-aem-to-a-different-port-or-context-path}

如果您要部署 [Experience Manager到应用程序服务器](/help/sites-deploying/application-server-install.md) 并启用Dynamic Media，您必须配置 **自域** 在Externalizer中。 否则，无法为Dynamic Media资源正确生成资源的缩略图。
...
在Experience ManagerQuickStart WAR部署中，无法派生端口号和上下文路径，因此您必须配置 **自域**. 请参阅 [Externalizer文档](/help/sites-developing/externalizer.md) 关于如何配置 **自域**.
...
1. 在命令行上，启动快速启动时，可以执行以下任一操作：
   ...
   * 不添加 `-r dynamicmedia` 到命令行中启动jar文件。
...
1. 请求 `https://localhost:4502/is/image`. 您会收到一条消息，指出Dynamic Media已禁用。
   ...
   >[!NOTE]
   >
   禁用Dynamic Media运行模式后，用于生成 `cqdam.pyramid.tiff` 将自动跳过节目。 它还禁用动态演绎版支持和其他Dynamic Media功能。
   >
...
如果您要将Experience Manager- Dynamic Media从6.3升级到6.5（现在包括零停机部署功能），则必须运行以下curl命令。 该命令会迁移您的所有预设和配置，从 `/etc` 到 `/conf` CRXDE Lite中。
...
Dynamic Media图像投放的工作方式为：从Experience Manager创作中发布图像资产（包括视频缩略图），并将其复制到Adobe的按需复制服务（复制服务URL）。 然后，通过按需图像投放服务（图像服务URL）来投放资产。
...
需要超过最大内存限制的映像将被拒绝。
>
要更改PTIFF创建的内存限制，请导航至 **[!UICONTROL 工具]** > **[!UICONTROL 操作]** > **[!UICONTROL Web控制台]** > **[!UICONTROL Adobe CQ Scene7 PTiffManager]** 并更改 **[!UICONTROL maxMemory]** 值。

### 设置身份验证 {#setting-up-authentication}

在创作实例上设置复制身份验证，以便将图像复制到Dynamic Media图像投放服务。 首先获取一个KeyStore，然后将其保存在下 **[!UICONTROL dynamic-media-replication]** 用户并对其进行配置。 您的公司管理员在配置过程中收到一封欢迎电子邮件，其中包含KeyStore文件和必要的凭据。 如果您没有收到此信息，请联系Adobe客户支持。
...
1. 在“用户管理”页面上，导航至 **[!UICONTROL dynamic-media-replication]** 用户，然后选择以打开。
...
1. 输入密码并在 **[!UICONTROL 设置KeyStore访问密码]** 对话框。
...
1. 在 **[!UICONTROL 编辑Dynamic-media-replication的用户设置]** 页面，展开 **从KeyStore文件添加私钥** 区域并添加以下内容（请参阅后面的图像）：
   ...
   * 在 **[!UICONTROL KeyStore文件密码]** 字段中，输入KeyStore文件密码。 此密码为 **非** 您在步骤5中创建的KeyStore密码，但在预配期间发送给您的欢迎电子邮件中提供的KeyStore文件密码Adobe。 如果您未收到KeyStore文件密码，请联系Adobe客户支持。
...
1. 选择 **[!UICONTROL 设置]** 选项卡，然后输入以下内容：
   ...
   * **[!UICONTROL 密钥存储别名]**  — 此值与 **新建别名** 在中生成键时设置的值 [设置身份验证](#setting-up-authentication)；例如， `replication`. （请参阅中的步骤7） [设置身份验证](#setting-up-authentication).)
...
>
* 检查复制日志，确保已复制资产。
* 发布图像。 选择图像并选择 **[!UICONTROL 查看器]** 在下拉菜单中，选择查看器预设。 选择 **[!UICONTROL URL]**. 要验证您是否可以看到图像，请在浏览器中复制并粘贴URL路径。
>
...
此问题可能是由未能为设置KeyStore所导致 `dynamic-media-replication` 用户。
...
**解决方案：**
检查 `KeyStore` 保存到 **dynamic-media-replication** 并且向提供了正确的密码。
...
1. 在“用户管理”页面上，导航至 `dynamic-media-replication` 用户，然后选择以打开。
1. 选择 **[!UICONTROL 密钥库]** 选项卡。 如果 **[!UICONTROL 创建密钥库]** 按钮时，您必须重做以下步骤 [设置身份验证](#setting-up-authentication) 更早。
1. 如果必须重做KeyStore设置，则必须这样做 [配置复制代理](/help/assets/config-dynamic.md#configuring-the-replication-agent) 同样的。
...
* 添加了图像预设。
* 配置 **[!UICONTROL Dynamic Media配置（早于6.3版本）]** 在Cloud Service下。 此测试需要图像服务URL
...
1. 通过打开图像并点按来导航到该图像的演绎版 **[!UICONTROL 节目]**.
...
**要配置Dynamic MediaCloud Service，请执行以下操作：**

1. 在Experience Manager中，选择Experience Manager徽标以访问全局导航控制台，然后导航到 **[!UICONTROL 工具]** > **[!UICONTROL Cloud Service]** > **[!UICONTROL Dynamic Media配置（早于6.3版本）]**.
1. 在Dynamic Media配置浏览器页面的左侧窗格中，选择 **[!UICONTROL 全局]**，然后选择 **[!UICONTROL 创建]**.
1. 在 **[!UICONTROL 创建Dynamic Media配置]** 对话框中，在标题字段中键入标题。
...
**何时使用：** 在配置Dynamic Media配置（早于6.3版本）时，将会启动多项功能，包括视频报告。 该配置会在地区性Analytics公司中创建报表包。 如果配置多个作者节点，请为每个节点创建单独的报表包。 因此，各安装之间的报表数据不一致。 此外，如果每个作者节点引用同一混合发布服务器，则上次的作者安装操作会更改所有视频报表的目标报表包。 此问题会使Analytics系统过载并且导致报表包过多。

**开始使用：** 通过完成以下三个任务来配置视频报表。

1. 在第一个创作节点上配置Dynamic Media配置（早于6.3版本）后，创建Video Analytics预设包。 此初始任务很重要，因为它允许新配置继续使用同一报表包。
1. 将Video Analytics预设包安装到任意 ***新建*** 创作节点 ***早于*** 您可以配置Dynamic Media配置（早于6.3版本）。
1. 验证并调试软件包安装。
...
确保您完成了此任务 ***早于*** 您可以配置Dynamic Media配置（早于6.3版本）。 否则，会导致创建另一个未使用的报表包。 此外，即使视频报表可继续正常使用，数据收集仍未得到优化。
...
要通过JCR检查Video Analytics预设，您必须有权访问CRXDE Lite。
     ...
     如果您无权在“创作”节点上访问“CRXDE Lite”，则可以通过“发布”服务器检查预设。
...
如果您将Dynamic Media用于生产中的(1)图像处理 *或* (2)成像和视频，然后可以使用Adobe按原样提供的默认滤镜。 默认情况下，以下筛选器处于活动状态：
 ...
 <tbody>
  ...
  <tr>
   ...
   <td>开箱即用的“过滤器图像”（适用于单个图像资产，包括交互式图像）和“过滤器集”（适用于旋转集、图像集、混合媒体集和轮播集）将：
    <ul>
     <li>包含PTIFF图像和元数据以进行复制(任何以 <strong>cqdam</strong>)。</li>
  ...
  <tr>
   ...
   <td>现成的“filter-video”将：
    <ul>
     ...
     <li>从复制中排除原始视频和静态缩略图呈现。<br /> <br /> <strong>注意：</strong> 代理视频演绎版不包含二进制文件，而是只是节点属性。 因此，对发布者存储库大小没有影响。</li>
    </ul> </td>
  </tr>
  <tr>
   ...
   <td><p>您可以配置传输URI以指向Experience Manager发布服务器，而不是AdobeDynamic Media云复制服务URL。 设置此过滤器后，Dynamic Media Classic可以交付资源而不是Experience Manager发布实例。</p> <p>现成的“filter-images”、“filter-set”和“filter-video”将：</p>
    <ul>
     <li>包括PTIFF图像、代理视频呈现版本和用于复制的元数据。 但是，由于它们不存在于JCR中(对于运行Experience Manager的用户而言)，因此Dynamic Media Classic集成 — 它实际上不执行任何操作。</li>
...
1. 定位 **[!UICONTROL filter-video]**，右键单击该文件夹，然后选择 **[!UICONTROL 复制]**.
1. 在左侧文件夹树中，导航到 `/etc/replication/agents.author/publish`
1. 定位 `jcr:content`，右键单击该文件夹，然后选择 **[!UICONTROL 粘贴]**.
...
1. 定位 **[!UICONTROL 滤镜图像]**，右键单击该文件夹，然后选择 **[!UICONTROL 复制]**.
1. 在左侧文件夹树中，导航到 `/etc/replication/agents.author/publish`
1. 定位 `jcr:content`，右键单击该文件夹，然后转到 **[!UICONTROL 创建]** > **[!UICONTROL 创建节点]**. 输入名称 `damRenditionFilters` 类型 `nt:unstructured`.
1. 定位 `damRenditionFilters`，右键单击该文件夹，然后选择 **[!UICONTROL 粘贴]**.
...
>
您是否在一个服务器上使用多个过滤器？ 例如，一个筛选器用于复制到发布，另一个筛选器用于s7delivery。 如果是这样，则必须确保这两个过滤器具有不同的值 **userId** 分配给 `jcr:content` 节点。 请参阅下面的图像：
...
1. 要定义过滤器的Mime类型，您可以按如下方式查找Mime类型：
   ...
   下图是资产路径的一个示例， `dc:format`.

   ![chlimage_1-512](assets/chlimage_1-512.png)

   请注意 `dc:format` （对于资产） `Fiji Red.jpg` 是 `image/jpeg`.

   要使此滤镜应用于所有图像，而不管其格式如何，请将该值设置为 `image/*` 位置 `*` 是应用于任何格式的所有图像的正则表达式。

   要使滤镜仅应用于文字JPEG的图像，请输入值 `image/jpeg`.
...
**先决条件** - *早于* 配置Dynamic Media映像服务器时，请确保您的Windows®虚拟机包括Microsoft® Visual C++库的安装。 运行Dynamic Media Image Server时需要这些库。 您可以 [在此处下载Microsoft® Visual C++ 2010可再发行软件包(x64)](https://www.microsoft.com/en-us/download/details.aspx?id=26999).

要配置Dynamic Media图像服务器设置，请执行以下操作：

1. 在Experience Manager的左上角，选择 **[!UICONTROL Adobe Experience Manager]** 要访问全局导航控制台，请导航至 **[!UICONTROL 工具]** > **[!UICONTROL 操作]** > **[!UICONTROL Web控制台]**.
...
1. 在包列表中，在Adobe CQ Scene7 ImageServer的右侧，选择 **[!UICONTROL 编辑]** 图标。
...
默认清单配置的位置取自 **[!UICONTROL 目录根]** 的默认值 **[!UICONTROL Adobe CQ Scene7平台服务器]** 捆绑。 默认情况下，此值位于 **[!UICONTROL 工具]** > **[!UICONTROL 常规]** > **[!UICONTROL CRXDE Lite]**
...
完成更改默认清单后，在页面的左上角选择 **[!UICONTROL 全部保存]**.

确保选择 **[!UICONTROL 访问控制]** 选项卡（“属性”选项卡的右侧），然后将访问控制权限设置为 `jcr:read` 适用于每个用户和dynamic-media复制用户。
...
高级用例可以使用手动配置 `icc=` 修饰符，用于明确选择输出颜色配置文件：
...
1. 导航到 [Software Distribution](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html) 并下载 `cq-6.3.0-featurepack-12445`.
   ...
   请参阅 [如何使用包](/help/sites-administering/package-manager.md) 有关在中使用包的详细信息 [!DNL Adobe Experience Manager].
...
1. 在 **[!UICONTROL 工具]** > **[!UICONTROL 常规]** > **[!UICONTROL CRXDE Lite]**，导航到 `/conf/global/settings/dam/dm/imageserver/jcr:content` ，其中包含默认的Adobe Color配置文件。
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>字符串</td>
   <td>相对</td>
   <td><p>指定渲染方法。 可接受的值为： <strong>可感知、相对、饱和度、绝对。 </strong><i></i>Adobe推荐 <strong>相对 </strong><i></i>作为默认值。</p> </td>
  </tr>
 </tbody>
</table>
...
例如，您可以设置 **[!UICONTROL iccprofilergb]** 到 `sRGB`、和 **[!UICONTROL iccprofilecmyk]** 到 **[!UICONTROL WebCoat]**.
...
请参阅 [交付Dynamic Media资源](/help/assets/delivering-dynamic-media-assets.md).
 ...
 <tbody>
  ...
  <tr>
   ...
   <td><p>复制嵌入代码对话框会显示类似于以下内容的代码片段（代码示例仅用于演示目的）：</p> <p><code class="code">&lt;style type="text/css"&gt;
       ...
       &lt;/script&gt;</code></p> <p>位置 <code>PUBLISHNODE</code> 是指常规Experience Manager发布节点和 <code>IMAGESERVICEPUBLISHNODE</code> 是指图像服务URL。</p> <p>另请参阅 <a href="/help/assets/delivering-dynamic-media-assets.md">交付Dynamic Media资源</a>.</p> </td>
  </tr>
 </tbody>
</table>
