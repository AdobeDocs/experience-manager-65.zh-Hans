...
description: 了解如何在中管理资源到期状态和已许可资源的信息 [!DNL Experience Manager].
contentOwner: AG
role: User, Admin
...
您可以在以下位置查看资源的到期状态： [!DNL Assets] 在卡片视图和列表视图中均使用控制台。
...
您还可以在以下位置查看资源的到期状态： **[!UICONTROL 引用]** 边栏。 它管理资产过期状态以及复合资产与引用的子资产、收藏集和项目之间的关系。
...
1. 在 [!DNL Assets] 控制台，单击 **[!UICONTROL Search]** 工具栏以显示Omnisearch框。
...
当您选择 **[!UICONTROL 已过期]** 选项，则 [!DNL Assets] 控制台仅显示由复合资源引用的已过期资源和子资源。 引用过期子资产的复合资产不会在子资产过期后立即显示。 相反，它们会在以下时间后显示 [!DNL Experience Manager] 检测在下次调度程序运行时是否引用过期的子资源。
...
1. 选择 **[!UICONTROL Adobe CQ DAM到期通知]**. 默认情况下， **[!UICONTROL 基于时间的计划程序]** 选择，这将安排在特定时间检查资产是否已过期子资产的作业。 作业完成后，具有过期子资产和引用资产的资产会在搜索结果中显示为已过期。
...
1. 选择 **[!UICONTROL 发送电子邮件]** 资产过期时接收电子邮件。
   ...
   >[!NOTE]
   >
   >仅限资产创建者（将特定资产上传到的人员） [!DNL Assets])会在资产过期时收到电子邮件。 请参阅 [如何配置电子邮件通知](/help/sites-administering/notification.md) 有关总体配置电子邮件通知的其他详细信息 [!DNL Experience Manager] 级别。

1. 在 **[!UICONTROL 提前通知（以秒为单位）]** 字段，指定当您希望接收有关到期的通知时，资产到期之前的时间（以秒为单位）。 资源创建者在资源到期前收到一条消息，通知您资源将在指定时间后到期。 资产过期后，您将收到另一通知来确认过期。 此外，还会停用过期的资产。
...
1. 要显示其资源详细信息页面，请在 [!DNL Assets] 界面中，选择资产并单击 **[!UICONTROL 属性]** ![查看属性](assets/do-not-localize/info-circle-icon.png).
...
1. 要根据资产的状态搜索资产，请单击 **[!UICONTROL Search]** ![搜索选项](assets/do-not-localize/search_icon.png) 以显示Omnisearch栏。
1. 选择 `Return` 并单击 [!DNL Experience Manager] 以显示“搜索”面板。
...
此功能强制接受许可协议，然后才能从下载许可资产 [!DNL Adobe Experience Manager Assets].
...
>
>如果创建或修改许可证页，或者从上一页将其移植 [!DNL Experience Manager] 对于版本，Adobe建议将它们存储在 `/apps/settings/dam/drm/licenses` 或 `/conf/&ast;/settings/dam/drm/licenses`.

### 下载受DRM保护的资产 {#downloading-drm-assets}

1. 在信息卡视图中，选择要下载的资源并单击 **[!UICONTROL 下载]**.
1. 在&#x200B;**[!UICONTROL 版权管理]**&#x200B;页面，从列表中选择要下载的资产。
1. 在 [!UICONTROL 许可证] 窗格，选择 **[!UICONTROL 同意]**. 资产旁边会显示一个复选标记。 单击 **[!UICONTROL 下载]** 选项。

   >[!NOTE]
   >
   >此 **[!UICONTROL 下载]** 仅当您选择同意受保护资产的许可协议时，才会启用选项。 但是，如果您的选择包含受保护和不受保护的资产，则只有受保护的资产会列在窗格和 **[!UICONTROL 下载]** 选项下，即可下载不受保护的资产。 要同时接受多个受保护资产的许可协议，请从列表中选择资产，然后选择“同 **[!UICONTROL 意”]**。
