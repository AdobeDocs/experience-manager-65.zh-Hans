...
![在“过滤器的元数据验证谓词”面板中选择的选项](assets/invalid-metadata-predicate.png)
...
1. 键值对是提供给表单用户的选项。 您可以手动或从JSON文件提供键值对。
   ...
   * 要手动指定值，请选择 **[!UICONTROL 手动添加]**，然后单击 **[!UICONTROL 添加选项]** 并指定选项文本和值。 例如，指定视频、PDF、Word和图像资源类型。
...
1. 单击 **[!UICONTROL 添加规则]** 并选择 **[!UICONTROL 资源类型]** 创建依赖项的字段。 还可以选择创建依赖关系时所依据的字段值。 在这种情况下，请选择“ **[!UICONTROL 视频]**”。 单击&#x200B;**[!UICONTROL 完成]**&#x200B;以保存更改。
...
1. 编辑自定义表单。 添加必填字段。 例如，添加 **[!UICONTROL 类别]** 字段并将字段设为必填。
   ...
   ![通过在元数据架构表单编辑器的“规则”选项卡中选择“必填”，将必填字段添加到元数据表单](assets/mandatory-field-metadata-schema-editor.png)
...
1. （可选）访问 `https://[aem_server]:[port]/system/console/components/`. 配置并启用 `com.day.cq.dam.core.impl.MissingMetadataNotificationJob` 默认禁用的组件。 设置频率 [!DNL Experience Manager] 检查资源上的元数据的有效性。 此配置添加一个属性 `hasValidMetadata` 到 `jcr:content` 个资产。 [!DNL Experience Manager] 使用此属性筛选搜索结果中的无效资源。 如果在检查后添加资产，则该资产不会使用 `hasValidMetadata` 直到下一次计划检查。 因此，直到下一次计划检查之后，资产才会出现在无效元数据的搜索筛选器中。
