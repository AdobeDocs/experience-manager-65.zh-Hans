...
只有在以下情况下才能链接到URL： *非* 将Experience Manager用作WCM。 当您要以弹出窗口或模式窗口形式交付视频播放器时，将使用链接（与嵌入）。 如果您将Experience Manager用作WCM， [直接在页面上添加资产](adding-dynamic-media-assets-to-pages.md).
...
另请参阅 [上传资源](manage-assets.md#uploading-assets).
...
1. 导航至 *已发布* 要复制其图像预设URL或查看器预设URL的资产，然后选择要打开的资产。
...
1. 根据您选择的资产，执行以下操作之一：
   ...
   * 如果选择了图像，请在下拉菜单中选择 **[!UICONTROL 节目]**.

     在 **[!UICONTROL 动态]** 标题，选择预设名称以在右框架中查看其演绎版。 如有必要，请滚动格式副本列表以查看动态标题。
...
1. 导航至 *已发布* 要复制URL的静态资源，然后选择要将其打开的资源。
...
1. 在 **[!UICONTROL Cloud Service]** 页面，向下滚动到 **[!UICONTROL Dynamic MediaCloud Service]** 标题，然后选择 **[!UICONTROL 显示配置]**.
1. 下 **[!UICONTROL 可用配置]**，选择所需配置的名称。

1. 在 **[!UICONTROL Dynamic Media云设置]** 页面，在 **[!UICONTROL 视频服务URL]**，复制整个URL路径。 在后面的步骤中，您需要使用复制的URL路径。
...
1. 在页面的左上角附近，选择 **[!UICONTROL Cloud Service]**，然后选择Experience Manager徽标并导航到 **[!UICONTROL 常规]** > **[!UICONTROL CRXDE Lite]**.
...
1. 在 **[!UICONTROL Dynamic MediaCloud Service设置]** 页面上，执行以下操作：
   ...
   * 下 **[!UICONTROL 注册ID]**，复制ID最后一部分中的客户名称。 在稍后这些步骤中，您需要复制的客户名称。
