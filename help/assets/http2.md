...
1. 要切换到HTTP/2，请启动Adobe客户支持请求。 请参阅[打开支持票证](https://experienceleague.adobe.com/?support-solution=General&lang=en&support-tab=home#support)。
   ...
   1. 在您的支持请求中提供以下信息：
      ...
      1. 主要联系人姓名、电子邮件、电话。
      1. 要转换为HTTP/2的所有域。
      1. 确认您使用安全HTTPS处理富媒体请求。
      1. 确认您通过Adobe使用CDN，且没有通过直接关系进行管理。
   ...
   1. 完成后，您将收到通知，并且可以验证是否成功过渡到HTTP2。
      ...
      对于Firefox和Chrome，有一个名为“HTTP/2和SPDY Indicator”的扩展。 浏览器仅安全支持http/2，因此有必要使用https调用URL进行验证。 如果支持http/2，则扩展将指示i。扩展采用蓝色Flash符号和标题`X-Firefox-Spdy`的形式： `h2`。

## 我何时可以开始过渡到HTTP/2？ {#when-can-i-expect-to-be-transitioned-over-to-http}
...
对于Firefox和Chrome，有一个名为“HTTP/2和SPDY Indicator”的扩展。 浏览器仅安全支持http/2，因此有必要使用https调用URL进行验证。 如果支持http/2，则扩展将指示该情况。 扩展采用蓝色Flash符号和标题`X-Firefox-Spdy`的形式： `h2`。
