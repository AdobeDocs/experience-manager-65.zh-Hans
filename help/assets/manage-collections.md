...
收藏集是中的一组资源 [!DNL Adobe Experience Manager Assets]. 使用收藏集可在用户之间共享资源。集合可以是静态集合或基于搜索结果的动态集合。
...
您可以使用以下任一方式创建收藏集 [静态引用](#creating-a-collection-with-static-references) 或基于 [基于搜索条件的过滤器](#creating-a-smart-collection). 您还可以从Lightbox创建收藏集。
...
1. 向收藏集添加成员并分配相应的权限。或者，选择&#x200B;**[!UICONTROL 公共收藏集]**，以允许所有用户访问该收藏集。
   ...
   >[!NOTE]
   >
   >要允许成员与其他用户共享收藏集，请提供 `dam-users` 路径中的组读取权限 `home/users`. 将权限授予以下位置的用户： `/content/dam/collections` 允许用户在弹出列表中查看收藏集的位置。 或者，让用户成为 `dam-users` 组。
...
1. 在 [!DNL A]资产用户界面，选择资产并单击 **[!UICONTROL 目标收藏集]** ![添加到收藏集](assets/do-not-localize/add-to-collection.png) 工具栏中。
或者，您可以将资产拖到 **[!UICONTROL 放入收藏集]** 界面上的区域。 当区域的标签变为 **[!UICONTROL 放置以添加]**.

1. 在 **[!UICONTROL 添加到收藏集]** 页面上，选择要将资产添加到的收藏集。
...
1. 单击&#x200B;**[!UICONTROL 保存]**。此 **[!UICONTROL 编辑智能收藏集]** 出现对话框。
...
1. 从 [!UICONTROL 收藏集] 在控制台中，选择一个收藏集并单击 **[!UICONTROL 属性]** 工具栏中。
...
1. 要查看特定收藏集的元数据属性，请取消选择收藏集列表中剩余的收藏集。 使用特定收藏集的元数据填充元数据编辑器字段。
   ...
   >* 在列表顶部，选中附近的复选框 **[!UICONTROL 标题]** 可在选择收藏集和清除列表之间切换。

1. 单击 **[!UICONTROL 保存并关闭]** 工具栏中，然后关闭确认对话框。
1. 要将新元数据附加到现有元数据，请选择 **[!UICONTROL 附加模式]**. 如果不选中此选项，则新元数据将替换字段中的现有元数据。单击&#x200B;**[!UICONTROL “提交”。]**

   >[!NOTE]
   >
   >为所选收藏集添加的元数据将覆盖这些收藏集的先前元数据。 使用 [!UICONTROL 附加模式] 在可以包含多个值的字段中向现有元数据添加新值。 始终会覆盖单值字段。 您在中添加的任何标记 [!UICONTROL 标记] 字段)会附加到元数据中的现有标记列表中。

自定义元数据 [!UICONTROL 属性] 页面，包括添加、修改和删除元数据属性，请使用架构编辑器。
...
1. 在对话框中，单击 **[!UICONTROL 删除]** 以确认删除操作。
   ...
   >[!NOTE]
   >
   >您也可以通过以下方式删除智能收藏集 [删除保存的搜索](#saved-searches).
...
1. 在 **[!UICONTROL 下载]** 对话框，请单击 **[!UICONTROL 下载]**. 如果要下载收藏集中资产的演绎版，请选择 **[!UICONTROL 节目]**. 选择 **[!UICONTROL 电子邮件]** 用于向收藏集所有者发送电子邮件通知的选项。

   选择要下载的收藏集时，将会下载收藏集下的完整文件夹层次结构。 要将您下载的每个收藏集（包括嵌套在父收藏集下的子收藏集中的资产）包含在单个文件夹中，请选择 **[!UICONTROL 为每个资源创建单独的文件夹]**.
...
1. 从 **[!UICONTROL 添加到收藏集]** 页面上，选择要添加集合的集合。

   >[!NOTE]
   >
   >默认情况下，将选择最近更新的收藏集，位于 **[!UICONTROL 添加到收藏集]** 页面。

1. 单击&#x200B;**[!UICONTROL 添加]**。此时将显示一条消息，确认该收藏集已添加到中的目标收藏集 **[!UICONTROL 选择目标]** 页面。 关闭消息以完成该过程。
...
创建智能收藏集时将创建保存的搜索。智能收藏集会自动添加到&#x200B;**[!UICONTROL 保存的搜索]**&#x200B;列表。此 [!UICONTROL 保存的搜索] 收藏集的查询保存在 `dam:query` CRXDE中位于相对位置的属性 `/content/dam/collections/`. 您可以保存的搜索以及列表中显示的已保存搜索没有限制。
