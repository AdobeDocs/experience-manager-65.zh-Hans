...
>[!NOTE]
>
>在 Adobe Target 中创建营销活动时，会为每个营销活动分配名为 `thirdPartyId` 的属性。 在 Adobe Target 中删除营销活动时，不会删除 thirdPartyId。 您不能为不同类型（AB、XT）的营销活动重复使用 `thirdPartyId`，也不能手动删除此属性。 要避免出现此问题，请为每个营销活动命名一个唯一的名称。 营销活动名称不能在不同的营销活动类型中重复使用。
>
...
此外，您还可以管理所有 Adobe Target 活动的目标和量度，以及管理 Adobe Target 受众。 此外，还包括Adobe Target活动报表（包括将A/B测试的入选者转化）。
...
1. 键入活动的名称。
   ...
   >[!NOTE]
   >
   >在创建活动并将某个Adobe Target云配置附加到页面或其某个父页面时，AEM会自动将Adobe Target视为引擎。
...
1. [锁定](#diagramtargetconfiguringtheaudiences)：指定每个体验所锁定的受众。 您可以定位特定受众。 如果您使用A/B测试，则可以决定哪个体验所占的流量百分比。
...
内容定位的“创建”步骤涉及创建体验。 在此步骤中，您可以创建或删除活动的体验，并将选件添加到每个体验。
...
在Target模式下，Target组件具有蓝色边框，并且放置目标消息指示目标特性。
...
[定位流程](/help/sites-authoring/content-targeting-touch.md#the-targeting-process-create-target-and-goals-settings)的“定位”步骤涉及将受众与您在“创建”步骤中创建的体验进行映射。 “目标”页显示每个体验定位的受众。 您可以指定或更改每个体验的受众。 如果您使用的是Adobe Target，则还可以创建A/B测试，该测试允许您针对特定体验定位某个受众的流量百分比。
...
1. 单击&#x200B;**确定。**
1. 输入百分比以配置将受众流量路由到每个体验的方式。 总和必须等于 100。
1. （可选）单击体验名称旁边的下拉菜单，以编辑体验名称。

## 目标和设置：配置活动并设置目标 {#goals-settings-configuring-the-activity-and-setting-goals}

[定位流程](/help/sites-authoring/content-targeting-touch.md#the-targeting-process-create-target-and-goals-settings)的“目标和设置”步骤涉及配置品牌活动的行为。 指定活动何时开始和结束以及活动优先级。 此外，您还可以跟踪目标。 具体来说，您可以决定要对活动衡量的指标。
...
有关成功量度的更多信息，请参阅[Adobe Target文档](https://experienceleague.adobe.com/docs/target/using/activities/success-metrics/success-metrics.html?lang=zh-Hans)。
