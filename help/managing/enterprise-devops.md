...
Adobe Experience Manager（AEM）部署通常包含多个环境，用于在不同级别实现不同目的：
...
* 每个开发人员的个人环境；通常驻留在其本地计算机上。在适当的时间间隔将代码与版本控制系统同步

根据系统的规模，开发环境可以同时具有创作实例和发布实例。

### 质量保证 {#quality-assurance}

该环境由质量保证团队使用，用于全面[测试](/help/sites-developing/test-plan.md)新系统的设计和功能。它应同时具有创作和发布环境（包含适当的内容），并提供所有必要的服务以启用完整的测试套件。
...
根据项目规模，它通常包括多个创作实例，或多个发布实例，或两者兼有。在较低的级别上，存储库也可群集到多个实例。
...
为了将发布环境中生成的数据反向复制到创作环境中，创作环境中的复制侦听器会轮询发布环境，并会从发布环境的反向复制发件箱中检索相关内容。

#### 发布 {#publish}

发布环境位于“隔离区”（DMZ）。这是访客访问和交互内容的环境（例如通过网站或移动应用程序），无论是面向公众，还是在企业内网中均是如此。发布环境：

* 拥有从创作环境复制的内容
* 向访客提供该内容
* 存储由访客生成的用户数据，例如评论或其他表单提交
...
代码（例如自定义的 Web 应用程序功能和设计模板）通过在不同内容存储库之间导出和导入包的方式进行传输。在需要的地方，可以将此复制配置为一个自动流程。
...
>[!NOTE]
>
>这并不意味着暂存内容必须持续与生产环境保持同步，定期更新即可，但尤其是在测试新一轮代码迭代之前进行更新。QA 和开发环境中的内容无需频繁更新，应能良好地代表生产环境的内容。

内容可以通过以下方式进行传输：

* 在不同环境之间——通过导出和导入包。
* 在不同实例之间：直接复制内容（[AEM 复制](/help/sites-deploying/replication.md)），通过 HTTP 或 HTTPS 连接完成。
