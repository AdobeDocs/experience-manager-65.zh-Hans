...
页面模板组件(例如， `head.jsp, body.jsp`)需要JSP包含才能加载ContextHub和Adobe Analytics集成(它是Cloud Service的一部分)。 全部包括加载JavaScript文件。

ContextHub条目应直接包含在 `<head>` 标记，而Cloud Service应包含在 `<head>` 并且在 `</body>` 部分；例如：
...
此 `contexthub` 您插入到的脚本位于 `<head>` 元素将ContextHub功能添加到页面中。

此 `cloudservices` 您在中添加的脚本 `<head>` 和 `<body>` 部分适用于添加到该页面的云服务配置。 (如果页面使用多个Cloud Service配置，则必须仅包含一次ContextHub jsp和Cloud Servicejsp。)

将Adobe Analytics框架添加到页面时， `cloudservices` 脚本会生成与Adobe Analytics相关的JavaScript以及对客户端库的引用，类似于以下示例：
...
* 使用data-tracking属性可在页面加载时跟踪事件数据。 (请参阅 [跟踪页面加载的自定义事件](/help/sites-developing/extending-analytics.md#tracking-custom-events-on-page-load).)
...
* 记录名为的值 `product` 特定结构的URL。
* 公开的数据成员 `product` 值，以便可以在Adobe Analytics框架中使用Adobe Analytics变量进行映射。
...
>[!NOTE]
>
>当没有事件与产品值相关联时，Adobe Analytics使用 `prodView` 事件。
...
例如，电子商务 `submitorder` 组件限制 `product` 个项目。 当购物车包含四个以上的产品时，将生成多个 `CQ_Analytics.record` 函数。
