...
页面模板组件（例如`head.jsp, body.jsp`）需要JSP include才能加载ContextHub和Adobe Analytics集成(它是Cloud Service的一部分)。 全部包括加载JavaScript文件。

ContextHub条目应直接包含在`<head>`标记的下方，而Cloud Service应包含在`<head>`中和`</body>`部分之前；例如：
...
您在`<head>`和`<body>`部分中添加的`cloudservices`脚本适用于添加到该页面的云服务配置。 (如果页面使用多个Cloud Service配置，则必须仅包含一次ContextHub jsp和Cloud Servicejsp。)
...
将通过Adobe Analytics启用跟踪的组件添加到Sidekick，以便将其添加到框架中。
