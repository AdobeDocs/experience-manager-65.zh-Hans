...
页面模板组件（例如`head.jsp, body.jsp`）需要JSP include才能加载ContextHub和Adobe Analytics集成（它是Cloud Services的一部分）。 全部包括加载JavaScript文件。

ContextHub条目应直接包含在`<head>`标记下方，而云服务应包含在`<head>`中和`</body>`部分之前；例如：
...
您在`<head>`元素之后插入的`contexthub`脚本将ContextHub功能添加到页面中。

您在`<head>`和`<body>`部分中添加的`cloudservices`脚本适用于添加到该页面的云服务配置。 （如果页面使用多个Cloud Services配置，则必须仅包含ContextHub jsp和Cloud Services jsp一次。）
...
启用AEM组件以与Adobe Analytics框架交互。 然后，配置您的框架，以便Adobe Analytics跟踪组件数据。

在编辑框架时，与Adobe Analytics框架交互的组件会显示在Sidekick中。 将组件拖到框架中后，将显示组件属性，然后可以使用Adobe Analytics属性映射这些属性。 （请参阅[为基本跟踪设置框架](/help/sites-administering/adobeanalytics-connect.md#creating-a-adobe-analytics-framework)。）

当组件具有名为`analytics`的子节点时，组件可以与Adobe Analytics框架进行交互。 `analytics`节点具有以下属性：

* `cq:trackevents`：标识组件公开的CQ事件。 （请参阅自定义事件。）
* `cq:trackvars`：命名使用Adobe Analytics属性映射的CQ变量。
* `cq:componentName`： Sidekick中显示的组件的名称。
* `cq:componentGroup`： Sidekick中包含该组件的组。

组件JSP中的代码会将JavaScript添加到触发跟踪的页面，并定义跟踪的数据。 JavaScript中使用的事件名称和数据名称必须匹配`analytics`节点属性的相应值。
...
使用这些数据跟踪方法时，Adobe Analytics集成模块会自动执行Adobe Analytics调用，以记录事件和数据。

### 示例：跟踪topnav点击量 {#example-tracking-topnav-clicks}

扩展基础topnav组件，以便Adobe Analytics跟踪页面顶部的导航链接点击量。 单击导航链接时，Adobe Analytics会记录所单击的链接以及单击的页面。
...
将topnav组件复制到CQ应用程序。 此过程要求在CRXDE Lite中设置您的应用程序。

1. 右键单击`/libs/foundation/components/topnav`节点，然后单击“复制”。
...
将启用了通过Adobe Analytics进行跟踪的组件添加到Sidekick，以便您可以将其添加到框架中。
...
1. 在Sidekick上，单击“设计”按钮。
   ...
   ![设计按钮，以直角正方形为特征。](assets/chlimage_1a.png)
...
topnav组件现已与Adobe Analytics Framework集成。 将组件添加到页面时，单击顶部导航栏中的项目会导致将跟踪数据发送到Adobe Analytics。
...
* 公开`product`值的数据成员，以便它们可以与Adobe Analytics框架中的Adobe Analytics变量进行映射。
...
Adobe Analytics集成模块使用AEM组件生成的`product`值构造`s.products`变量。 AEM组件生成的JavaScript中的`product`值是具有以下结构的值的数组：
...
如果`product`值中省略了某个数据项，则会在s.products中将该数据项作为空字符串发送。

>[!NOTE]
>
>当没有事件与产品值关联时，Adobe Analytics默认使用`prodView`事件。
...
通常，Web浏览器会限制GET请求的大小。 由于CQ产品和SKU值是存储库路径，因此包含多个值的产品阵列可能会超过请求大小限制。 因此，组件应限制每个`CQ_Analytics.record function`的`product`数组中的项数。 如果必须跟踪的项目数可能超过限制，请创建多个函数。

例如，电子商务`submitorder`组件将调用中的`product`项数限制为4。 当购物车包含四个以上的产品时，它将生成多个`CQ_Analytics.record`功能。
