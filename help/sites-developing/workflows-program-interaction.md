...
<table>
 <tbody>
  ...
  <tr>
   <td><code>POST</code></td>
   <td>完成其URI作为参数发送的工作项 <code>item</code> 并将相应的工作流实例前进到下一个节点，该节点由参数定义 <code>route</code> 或 <code>backroute</code> 如果倒退一步。<br /> 如果参数 <code>delegatee</code> 发送，即由参数标识的工作项 <code>item</code> 委托给指定的参与者。</td>
  </tr>
 </tbody>
</table>
