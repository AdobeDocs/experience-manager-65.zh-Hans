...
创建移动站点与创建标准站点类似，因为它还包括创建模板和组件。 有关创建模板和组件的更多详细信息，请参阅以下页面：[模板](/help/sites-developing/templates.md)、[组件](/help/sites-developing/components.md)和[AEM Sites开发入门](/help/sites-developing/getting-started.md)。 主要区别在于在站点中启用Adobe Experience Manager (AEM)内置的移动功能。 这是通过创建依赖于移动页面组件的模板来实现的。

考虑使用[响应式设计](/help/sites-developing/responsive.md)，创建适合多种屏幕大小的单个网站。

若要开始，您可以查看AEM中提供的&#x200B;**We.Retail移动演示网站**。
...
如果从前端缓存中提供`coolpage.feature.html`，AEM将没有机会分析该请求以找出移动设备选择器与新的用户代理不匹配，并且Bob获得的表示错误。
