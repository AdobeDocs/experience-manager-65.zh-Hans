...
页面上介绍了组件的基本结构 [AEM组件 — 基础知识](/help/sites-developing/components-basics.md#structure)，同时应用触屏界面和经典UI。 即使您不需要在新组件中使用触屏UI的设置，也可以在继承现有组件时了解这些设置。
...
因此 `global.jsp` 应包含在每个组件渲染JSP脚本中，其中在中提供的一个或多个对象 `global.jsp` 使用。
...
>[!NOTE]
>
>路径 `/libs/wcm/global.jsp`由CQ 5.3版本及更早版本使用的，现已过时。

### global.jsp、使用的API和Taglibs的功能 {#function-of-global-jsp-used-apis-and-taglibs}

下面列出了默认设置中提供的最重要的对象 `global.jsp`：
...
为了帮助解决此问题，AEM提供了 **客户端库文件夹**，可将客户端代码存储在存储库中，将其整理到不同类别中，并定义何时以及如何向客户端提供每种类别的代码。 然后，客户端库系统负责在最终网页中产生正确的链接，以加载正确的代码。
...
要基于现有组件为AEM开发新组件，可以复制组件，为新组件创建JavaScript文件，并将其存储在AEM可访问的位置(另请参阅 [自定义组件和其他元素](/help/sites-developing/dev-guidelines-bestpractices.md#customizing-components-and-other-elements))：
...
1. 打开新的组件文件夹，然后进行所需的更改。 另外，删除文件夹中任何无关的信息。
   ...
   * 或者完全重新处理整个组件（如果需要）
   ...
   >* 经典UI使用 [ExtJS构件](https://developer.adobe.com/experience-manager/reference-materials/6-5/widgets-api/index.html)
...
>[!NOTE]
>
>要查看页面加载的时间统计信息，可以使用Ctrl-Shift-U — 和 `?debugClientLibs=true` 在URL中设置。
...
>[!NOTE]
>
>此示例基于Geometrixx示例内容，该内容不再随AEM一起提供，已被We.Retail取代。 查看文档 [We.Retail参考实施](/help/sites-developing/we-retail.md#we-retail-geometrixx) 了解如何下载和安装Geometrixx。
...
1. 更改 `sling:resourceType` 图像的edit configuration节点的属性(属性： `textimage/cq:editConfig/cq:dropTargets/image/parameters/sling:resourceType`)到 `geometrixx/components/textimage.`

   这样，当图像放到页面上的组件时， `sling:resourceType` 扩展文本时间组件的属性设置为： `geometrixx/components/textimage.`

1. 修改组件的对话框以包含新选项。 新元件继承对话框中与原始元件相同的部分。 您所做的唯一补充是扩展 **高级** 选项卡，添加 **图像位置** 下拉列表，带有选项 **左侧** 和 **右**：

   * 离开 `textimage/dialog`属性未更改。

   注意如何 `textimage/dialog/items` 具有四个子节点，即tab1到tab4，表示“文本时间”对话框的四个选项卡。

   * 对于前两个选项卡（tab1和tab2）：

      * 将xtype更改为cqinclude（以继承自标准组件）。
      * 添加带有值的路径属性 `/libs/foundation/components/textimage/dialog/items/tab1.infinity.json`和 `/libs/foundation/components/textimage/dialog/items/tab2.infinity.json`、ID名称和ID名称等。
      * 删除所有其他属性或子节点。
   ...
   * 删除Tab4。
   ...
   图像位置作为保留在内容中 `imagePosition`节点的属性，表示 `textimage` 段落。 执行这些步骤后，组件对话框将如下所示：
...
1. 从复制标准图像组件 `/libs/foundation/components/image` 到Geometrixx组件文件夹中， `/apps/geometrixx/components`，使用图像作为目标节点名称。
