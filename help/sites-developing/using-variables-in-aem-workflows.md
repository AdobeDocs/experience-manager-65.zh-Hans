...
* [创建变量](/help/sites-developing/using-variables-in-aem-workflows.md#create-a-variable) ，基于要存储在其中的信息类型。
* [设置变量的值](/help/sites-developing/using-variables-in-aem-workflows.md#set-a-variable) 使用设置变量工作流步骤。
* [使用变量](/help/sites-developing/using-variables-in-aem-workflows.md#use-a-variable) 在OR Split和Goto AEM工作流步骤中，您可以定义用于做出路由选择决策的表达式。 您还可以在所有AEM Forms工作流步骤中使用变量。
...
变量是 [元数据映射](https://developer.adobe.com/experience-manager/reference-materials/6-5/javadoc/com/adobe/granite/workflow/metadata/MetaDataMap.html) 界面。 您可以使用 [元数据映射](https://developer.adobe.com/experience-manager/reference-materials/6-5/javadoc/com/adobe/granite/workflow/metadata/MetaDataMap.html) 在ECMAScript中访问使用变量保存的元数据。
...
1. 为变量指定可选描述，然后选择 ![在框内用复选标记指示的保存图标。](https://helpx.adobe.com/content/dam/help/en/experience-manager/6-4/forms/using/chart-component/Done_Icon.png) 以保存更改。 变量将显示在左侧窗格中可用的列表中。
...
1. 拖放 **设置变量** 步骤到工作流编辑器，选择该步骤，然后选择 ![“配置”图标由扳手指示。](https://helpx.adobe.com/content/dam/help/en/experience-manager/6-4/forms/using/configure_icon.png) （配置）。
1. 在设置变量对话框中，选择 **[!UICONTROL 映射]** > **[!UICONTROL 添加映射]**.
1. 在 **映射变量** 部分，选择要存储数据的变量，选择映射模式，然后指定要在变量中存储的值。 映射模式因变量类型而异。
1. 映射更多变量，以便生成有意义的表达式。 选择 ![在框内用复选标记指示的保存图标。](https://helpx.adobe.com/content/dam/help/en/experience-manager/6-4/forms/using/chart-component/Done_Icon.png) 以保存更改。

### 示例1：查询XML变量以设置字符串变量的值 {#example-query-an-xml-variable-to-set-value-for-a-string-variable}

选择要存储XML文件的XML类型变量。 查询XML变量以为XML文件中可用的属性设置字符串变量的值。 使用 **指定XML变量的XPATH** 字段，用于定义要在字符串变量中存储的属性。

在此示例中，选择 **formdata** 用于存储的XML变量 **cc-app.xml** 文件。 查询 **formdata** 变量，以便您可以为 **电子邮件地址** 字符串变量，用于存储值 **电子邮件地址** 中可用的属性 **cc-app.xml** 文件。
...
在此示例中，在定义路由表达式之前，使用 [示例2](/help/sites-developing/using-variables-in-aem-workflows.md#example2) 设置值 **totalvalue** 变量。 分支1处于活动状态，如果 **totalvalue** 变量大于50000。 同样，您可以定义一个规则以使Branch 2处于活动状态，如果 **totalvalue** 变量小于50000。
...
1. 选择 ![铅笔符号指示的编辑图标。](https://helpx.adobe.com/content/dam/help/images/en/edit.png) （编辑）图标（位于要编辑的变量名称旁）。
1. 编辑变量信息并选择 ![保存由复选标记指示的图标。](https://helpx.adobe.com/content/dam/help/en/experience-manager/6-4/forms/using/chart-component/Done_Icon.png) 以保存更改。 您无法编辑 **[!UICONTROL 名称]** 和 **[!UICONTROL 类型]** 变量对应的字段。
...
1. 在编辑工作流页面上，选择工作流模型Sidekick中可用的“变量”图标。 左窗格中的变量部分显示所有现有变量。
1. 选择要删除的变量名称旁边的删除图标。
1. 选择 ![由复选标记符号指示的“完成”图标。](https://helpx.adobe.com/content/dam/help/en/experience-manager/6-4/forms/using/chart-component/Done_Icon.png) 以确认并删除该变量。
