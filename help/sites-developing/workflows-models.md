...
1. 选择 **创建**，则 **创建模型**.
1. 此 **添加工作流模型** 出现对话框。 输入 **标题** 和 **名称** （可选）在选择 **完成**.
1. 新模型列在 **工作流模型** 控制台。
...
* [定义步骤](#addingasteptoamodel-) 和他们的 [参数](#configuring-a-workflow-step)
...
编辑 [**默认和/或旧版** （现成）工作流](#editing-a-default-or-legacy-workflow-for-the-first-time) 具有额外的步骤，以确保 [安全复制](/help/sites-developing/workflows-best-practices.md#locations-workflow-models) 在进行更改之前摄取。

更新完工作流后，您必须使用 **同步** 到 **生成运行时模型**. 请参阅 [同步您的工作流](#sync-your-workflow-generate-a-runtime-model) 以了解详细信息。
...
当您（或任何其他用户）对必须使用的工作流进行更改时 **同步** 生成运行时模型 — 即使单个对话框（例如，对于步骤）具有自己的保存选项。

当更改与运行时（保存的）模型同步时， **已同步** 将显示。

某些步骤具有必填字段和/或内置验证。 如果不满足这些条件，则尝试以下操作时将显示错误 **同步** 模型。 例如，尚未为定义参与者 **参与者** 步骤：
...
>[!NOTE]
>
>请参阅 [工作流模型的位置](/help/sites-developing/workflows-best-practices.md#locations-workflow-models) 以了解详细信息。
...
1. 打开现有的工作流模型进行编辑。 从 **工作流模型** 控制台，选择所需的模型，然后 **编辑**.
1. 打开步骤浏览器；使用 **切换侧面板**，该工具栏位于顶部工具栏的最左侧。 在此编辑器中，您可以：
...
1. 根据需要添加任意数量的步骤或其他更新。
   ...
   也可以复制、剪切、粘贴、分组或删除现有步骤；就像使用 [页面编辑器。](/help/sites-authoring/editing-content.md)

   也可以使用工具栏选项折叠/展开拆分步骤： ![wf-collapseexpand-toolbar-icon](assets/wf-collapseexpand-toolbar-icon.png)

1. 使用确认更改 **同步** （编辑器工具栏）以生成运行时模型。

   请参阅 [同步您的工作流](#sync-your-workflow-generate-a-runtime-model) 以了解详细信息。
...
1. 确认更改，方法： **保存并关闭**；后跟 **同步** （编辑器工具栏）以生成运行时模型。
...
如果经典UI中存在工作流模型，但在的选择弹出菜单中缺少该模型， **[!UICONTROL 时间线]** 然后，按照配置使其可用。 以下步骤说明了如何使用名为的工作流模型 **[!UICONTROL 请求激活]**.

1. 确认模型在触屏UI中不可用。 使用访问资源 `/assets.html/content/dam` 路径。 选择资源。 打开 **[!UICONTROL 时间线]** 在左边栏中。 单击 **[!UICONTROL 启动工作流]** 并确认 **[!UICONTROL 请求激活]** 弹出列表中不存在模型。

1. 导航至 **[!UICONTROL “工具”>“常规”>“标记”]**. 选择 **[!UICONTROL 工作流]**.
...
1. 使用完成此过程 **[!UICONTROL 同步]**. 现在，该工作流在触屏UI中可用。
...
>[!CAUTION]
>
>如果工作流暂存在中定义 **页面属性**，但不用于任何工作流步骤，则进度栏不会显示任何进度（无论当前工作流步骤是什么）。
...
1. 添加所需的（和职位） **暂存**. 您可以为工作流模型定义任意数量的阶段。
...
测试工作流时，好的做法是使用各种有效负载类型（包括与为其开发该工作流的类型不同的类型）。 例如，如果您打算让工作流处理资产，请通过将页面设置为有效负载来测试该工作流，并确保它不会引发错误。
...
您还可以配置要显示的AEM **调试** 日志文件中的消息。 请参阅 [记录](/help/sites-deploying/configure-logging.md) 当开发完成后，如需进一步资讯及了解详情，请将 **日志级别** 返回至 **信息**.
...
1. 删除 `Step 1` （因为对于本示例来说，它是错误的步骤类型）：

   * 单击该步骤并选择 **删除** 从组件工具栏中。 确认该操作。

1. 从 **工作流** 选择步骤浏览器，拖动 **参与者步骤** 放到工作流上并将其放置在 **流程开始** 和 **流程结束**.
1. 要打开“属性”对话框，请执行以下操作：

   * 单击参与者步骤并选择 **配置** 从组件工具栏中。
   * 双击参与者步骤。
...
1. 拖动 **或拆分** 放到工作流上并将其放置在 `Validate Content` 和 **流程结束**.
1. 打开 **或拆分** 进行配置。
1. 配置：
...
1. 将此工作流应用于您的页面，以便当用户移动到 **完成** 该 **验证内容** 步骤，用户可以选择是否要 **根据请求发布页面**，或 **拒绝发布请求**.
...
例如， **请求激活**. 此工作流用于发布中的页面 **站点** 当内容作者没有相应的复制权限时，将自动触发和。 请参阅 [自定义页面创作 — 自定义激活请求工作流](/help/sites-developing/customizing-page-authoring-touch.md#customizing-the-request-for-activation-workflow) 以了解更多详细信息。
