...
开箱即用 ` [CQ.Ext.tree.TreePanel](https://developer.adobe.com/experience-manager/reference-materials/6-5/widgets-api/index.html?class=CQ.Ext.tree.TreePanel)` 对象提供树形结构数据的用户界面表示。 使用 ExtJS Widgets **包中包含的**&#x200B;树概述组件显示了如何使用对象在`TreePanel`给定路径下方显示 JCR 树。窗口本身可以停靠/取消停靠。 在此示例中，窗口逻辑嵌入在组件 jsp 中的标记之间 &lt;script> 。
