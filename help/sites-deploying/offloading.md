...
卸载基于 [Apache Sling发现](https://sling.apache.org/documentation/bundles/discovery-api-and-impl.html) 和Sling JobManager功能。 要使用卸载，可以将Experience Manager集群添加到拓扑中，并确定集群处理的作业主题。 集群由一个或多个Experience Manager实例组成，因此单个实例被视为集群。
...
请参阅 [配置主题使用](/help/sites-deploying/offloading.md#configuring-topic-consumption) 以获取有关优化作业分配的信息。
...
1. 要禁用某个实例的主题使用，请在主题名称下方单击该实例旁边的禁用。
...
1. 单击主题旁边的以下按钮之一可配置实例的消费行为，然后单击保存：
   ...
   **注意：** 当您为某个主题选择“独占”时，其它所有主题都会自动设置为“已禁用”。
...
使用Web控制台或 `sling:OsgiConfig` 节点，以配置以下属性。 对象 `sling:OsgiConfig` 节点，作业使用者管理器服务的PID为org.apache.sling.event.impl.jobs.JobConsumerManager。
...
使用特定格式用于 ***名称*** 复制代理的属性，以便卸载框架自动为特定工作线程实例使用正确的代理。
...
1. 创建 **复制代理** 关于作者。 (请参阅 [复制代理文档](/help/sites-deploying/replication.md))。 指定任意 **标题**. 此 **名称** 必须遵循命名约定。
1. 使用以下属性创建代理：
...
1. 创建 **反向复制代理** 关于作者。 (请参阅 [复制代理文档](/help/sites-deploying/replication.md).) 指定任意 **标题**. 此 **名称** 必须遵循命名约定。
1. 使用以下属性创建代理：
...
1. 创建 **复制代理** 在辅助进程实例上。 (请参阅 [复制代理文档](/help/sites-deploying/replication.md).) 指定任意 **标题**. 此 **名称** 必须为 `offloading_outbox`.
1. 使用以下属性创建代理。
