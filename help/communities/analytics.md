...
>* **`event1`** 至 **`event7`**
>
>**在发布社区站点之前，**&#x200B;请务必通过移动在为社区站点启用AEM时自动映射到Analytics变量的Analytics变量来恢复预先存在的映射。
>
...
Sidekick不是必需的，可以最小化，这样它就不会阻止对报表包设置的访问。
...
* 指向UGC *的* SRP路径： `/content/usergenerated/asi/.../forum/jmtz-topic3`
...
>* **`event1`** 至 **`event7`**
>
>**则在发布社区站点之前，**&#x200B;恢复预先存在的映射。 将自动映射的AEM变量（为社区站点启用Analytics时）移动到其他Analytics变量。 所有Communities组件中的此重新映射都应保持一致。
>
>否则，可能会导致无法恢复的数据丢失。
