---
title: 问答论坛功能
description: 了解如何将问题与解答论坛功能添加到允许登录社区成员提问的页面。
...
问题与解答(Questions and Answers)论坛功能为社区成员提供了一个询问和回答问题的区域。 它允许成员：
...
* **已审核**
  ...
  如果选中，发布主题和注释必须先获得批准，然后才能显示在发布站点上。 默认值为取消选中。

* **已关闭**

  如果选中，论坛将关闭，且不会出现新的问题和评论。 默认值为取消选中。

* **富文本编辑器**

  如果选中，则可以使用标记输入主题和注释。 默认值为取消选中。
...
* **允许文件上传**
  ...
  如果选中，则允许将文件附件添加到问题或注释中。 默认值为取消选中。
...
* **允许固定**
  ...
  如果选中，可以将论坛主题固定到主题列表的顶部。 默认值为取消选中。
...
* **允许回复**
  ...
  如果选中，则允许对发布到该问题的评论进行回复。 默认值为取消选中。

* **允许投票**

  如果选中，请包含“投票”功能并附上问题。 默认值为取消选中。

* **允许用户删除评论和主题**

  如果选中，则允许成员删除他们发表的评论和问题。 默认值为取消选中。
...
* **关闭/重新打开主题**
  ...
  如果选中，受信任成员审查方可以关闭问题（主题）以进一步编辑和回答，也可以重新打开问题。 默认值为取消选中。

* **移动主题**
如果选中，则允许发布端审查方移动问题。 默认值为取消选中。

* **标记帖子**

  如果选中，则允许成员将其他人的问题或答案标记为不适当。 默认值为取消选中。
...
* **审核阈值**
  ...
  输入在通知版主之前，成员必须标记某个问题或答案的次数。 默认值为1（一次）。

* **标记限制**

  输入问题或答案在隐藏于公众视野之前必须标记的次数。 如果设置为–1，则标记的问题或答案永远不会从公开视图中隐藏。 否则，此数字必须大于或等于审核阈值。 默认值为5。
...
当登录用户具有审阅人或管理员权限时，他们能够执行组件配置允许的审阅任务，而不管该问题或答案的作者是谁。
...
未登录的网站访客只能读取已发布的问题和答案（如果支持），但无法添加问题或答案，也无法标记其他人的帖子。
