...
有关详细信息，请参阅Adobe隐私中心的[GDPR页面。](https://www.adobe.com/privacy/general-data-protection-regulation.html)
...
例如，要导出名为Weston McCall的用户UGC（该用户使用weston.mccall@dodgit.com作为可授权ID登录到communities站点），您可以发送类似于以下内容的http GET请求：
...
例如，要通过http-POST请求删除其可授权ID为`weston.mccall@dodgit.com`的用户的UGC，请使用以下参数：
...
要从Adobe Analytics中删除用户数据，请遵循[GDPR Analytics工作流程](https://experienceleague.adobe.com/docs/analytics/admin/data-governance/an-gdpr-workflow.html?lang=zh-Hans)，因为API不会从Adobe Analytics中删除用户数据。
...
例如，要通过http-POST请求删除其可授权ID为`weston.mccall@dodgit.com`的用户的配置文件，请使用以下参数：
...
>[!NOTE]
>
>`deleteUserAccount()` API仅禁用系统中的用户配置文件并删除UGC。 但是，要从系统中删除用户配置文件，请导航到`https://<server>:<port>/crx/de`上的&#x200B;**CRXDE Lite**，找到该用户节点并将其删除。
