...
可以将上下文审核配置为允许成员在标记帖子时选择原因。 可选择的标志原因列表是可配置的，包括是否可以输入自定义原因。 标记原因会与UGC一起保存，但原因不会触发任何特定操作。 只有标记的数量会触发通知。 已标记的内容将进行相应的批注，以便审查方可以对其进行处理。
...
用户生成的包含垃圾邮件词的帖子（涵盖所有内容类型，如博客、论坛和评论）在帖子上方标有“此帖子被分类为垃圾邮件”文本。

版主可以查看此类帖子，并将其标记为未出现在网站上。 可以在上下文中或通过批量审核UI对这些帖子执行审核操作。
...
情绪分析使用一组预先配置的规则并计算UGC的情绪。 默认规则位于`/libs/cq/workflow/components/workflow/social/sentiments/rules`。

规则生成的值介于1（所有负值，无正字）到10（所有正值，无负字）之间。 情绪值为5是中性情绪，且为默认值。
...
要覆盖或添加规则，请在/apps目录中按照默认规则的结构创建一组规则。 编辑情绪配置，以便标识规则的位置。

分析后，该情绪将与UGC一起存储。

在[批量审核控制台](/help/communities/moderation.md)中，可以根据情绪是负值、中性还是正值来筛选和查看UGC。

#### 标语 {#watchwords}

AEM Communities提供了&#x200B;*关注字分析器*&#x200B;作为评估[情绪](#sentiment)的过程中的步骤。 对标语提供的情绪值的贡献是由于对已发布内容中使用的负面和正面标语与禁止字词的比较造成的。

#### 配置情绪和标语 {#configure-sentiment-and-watchwords}

正面和负面标语列表可以自定义，情绪规则也可以自定义。
...
* **正面标语**
  ...
  以逗号分隔的单词列表，这些单词可构成覆盖默认值的正情绪。 默认列表为空。

* **负面标语**

  以逗号分隔的单词列表，这些单词会构成覆盖默认值的负情绪。 默认列表为空。
...
* **情绪规则**
  ...
  用于基于正和负标语计算情绪的规则的库位置。 默认值为`/libs/cq/workflow/components/workflow/social/sentiments/rules`（但是，不再涉及任何工作流）。
