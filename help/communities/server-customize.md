...
该接口定义了表示资源所必需的一组GETter。 重要的是，界面规定了Map&lt;String， Object> getAsMap()和String toJSONString()方法，这些方法对于渲染Handlebars模板并公开资源的GET JSON端点很有必要。
...
HTTP API POST端点是通过实现`SlingPostOperation`接口（包`org.apache.sling.servlets.post`）定义的PostOperation类。

`PostOperation`终结点实现将`sling.post.operation`设置为操作所回应的值。 所有参数设置为:operation的POST请求都委托给此实现类。
...
[Adobe Experience Cloud GitHub](https://github.com/Adobe-Marketing-Cloud)存储库中提供了示例代码。 搜索前缀为`aem-communities`或`aem-scf`的项目。
