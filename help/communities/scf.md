...
* 要修改JS模板或GET端点可用的信息，请执行以下操作：
   * 扩展 [社交组件](server-customize.md#socialcomponent-interface).
...
了解如何处理存储在中的UGC [社区内容存储](working-with-srp.md)，请参见：
...
访问 [服务器端自定义](server-customize.md) 有关自定义服务器端上Communities组件的业务逻辑和行为的信息。
...
在任一情况下，组件的 [所需的客户端库](clientlibs.md) 也必须出席。
...
包括元件是指将参照添加到 [“非现有”资源](srp.md#for-non-existing-resources-ners) （无JCR节点），例如使用脚本语言。
...
**使用Handlebars时** 模板化语言，使用包含非现有资源 [包含辅助函数](handlebars-helpers.md#include) 通过指定其resourceType：
...
请参阅 [SCF Handlebars助手](handlebars-helpers.md) 有关SCF中可用的自定义帮助程序的列表和说明。

## 客户端框架 {#client-side-framework}

### 模型视图JavaScript框架 {#model-view-javascript-framework}

该框架包括以下扩展 [Backbone.js](https://backbonejs.org/)，是一个模型视图JavaScript框架，便于开发丰富、交互式的组件。 面向对象的性质支持可扩展/可重用的框架。 HTTP API简化了客户端和服务器之间的通信。
...
要自定义客户端上Communities组件的外观和行为，请参考 [客户端自定义](client-customize.md)，其中包括有关以下项的信息：
