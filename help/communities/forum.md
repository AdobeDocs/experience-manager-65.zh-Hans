...
添加 `Forum` 组件到创作模式下的页面，请使用组件浏览器查找
...
当 [所需的客户端库](/help/communities/essentials-forum.md#essentials-for-client-side) 包括，这就是 `Forum` 组件出现：

![论坛 — 组件](assets/forum-component.png)

### 配置论坛 {#configuring-a-forum}

选择已放置的 `Forum` 组件，以便您能够访问和选择 `Configure` 图标打开“编辑”对话框。
...
* **允许关注**
  ...
  如果选中，则为论坛帖子提供以下功能，允许成员成为 [已通知](/help/communities/notifications.md) 新职位。 默认值为未选中。
...
>[!NOTE]
>
>可能有必要同时检查两者 `AllowThreaded Replies` 和 `Allow users to Delete Comments and Topics` 启用主题评论。
