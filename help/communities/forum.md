---
title: 论坛功能
description: 了解如何添加和配置论坛功能，该功能为登录的社区成员提供一个用于创建、查看、关注、搜索或回复主题的区域。
...
论坛功能为Publish环境中的登录站点访客（社区成员）提供了一个区域，用于：
...
要将`Forum`组件添加到创作模式下的页面，请使用组件浏览器来查找
...
当包含[所需的客户端库](/help/communities/essentials-forum.md#essentials-for-client-side)时，`Forum`组件的显示方式如下：

![论坛组件](assets/forum-component.png)

### 配置论坛 {#configuring-a-forum}

选择放置的`Forum`组件，以便您可以访问并选择用于打开“编辑”对话框的`Configure`图标。

![配置 — 新](assets/configure-new.png)

![论坛配置](assets/forum-config.png)
...
* **已关闭**
  ...
  如果选中，则论坛将关闭并显示新主题和评论。 默认值为未选中。

* **富文本编辑器**

  如果选中，则主题和注释可输入标记。 默认值为未选中。
...
* **允许关注**
  ...
  如果选中，则为论坛帖子提供以下功能，此功能允许成员接收新帖子的[通知](/help/communities/notifications.md)。 默认值为未选中。

* **允许固定**

  如果选中，论坛主题可能会固定到主题列表的顶部。 默认值为未选中。
...
>[!NOTE]
>
>可能需要同时选中`AllowThreaded Replies`和`Allow users to Delete Comments and Topics`才能启用有关主题的评论。
...
在&#x200B;**翻译**&#x200B;选项卡下，如果为社区站点启用了翻译，则可以将翻译设置为翻译整个主题或选定的帖子。
...
可在面向开发人员的[Forum Essentials](/help/communities/essentials-forum.md)页面上找到更多信息。
