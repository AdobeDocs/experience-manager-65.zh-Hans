...
在Angular中，控制器连接\$scope中的变量，使其公开给视图。 angular-app-controllers.js.jsp脚本遵循angular-app-module.js.jsp所示的模式，即遍历每个下级页面（包括本身），并输出每个页面定义的控制器片段(通过controller.js.jsp)。 它定义的模块称为 `cqAppControllers` 并且必须作为顶级应用程序模块的依赖项列出，以便提供页面控制程序。
...
为了解决此问题，PhoneGap应用程序使用单页应用程序(SPA)模式，以便基本URI（不包括哈希）永不更改。 因此，需要您引用的每个资源、模板或脚本 **必须相对于顶级页面。** 顶层页面通过初始化Angular路由和控制器 `*<name>*.angular-app-module.js` 和 `*<name>*.angular-app-controllers.js`. 此页面应该是距离存储库根目录*不*扩展sling：redirect的最近页面。
