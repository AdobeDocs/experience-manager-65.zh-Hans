...
description: 要开始新的Adobe Experience Manager (AEM)移动设备应用程序体验，需要先整合各种角色，然后才能编辑内容。 关注本页，以开始使用AEM Mobile On-Demand服务。
contentOwner: User
...
***AEM管理员***&#x200B;负责通过使用创建向导创建应用程序或导入现有应用程序将应用程序添加到AEM Mobile目录。 使用AEM的&#x200B;*创建向导*&#x200B;创建应用程序的AEM Mobile管理员通常会从Adobe的开箱即用参考示例或（通常）由&#x200B;*AEM开发人员创建的自定义应用程序模板中选择所需的应用程序模板之一。*
...
**AEM开发人员**&#x200B;扩展并创建自定义Web模板和组件，以使*AEM Author *能够创建精美动人的移动体验。 这些模板和组件不仅针对移动设备应用程序世界进行了优化，而且还与设备和AEM服务器（任何远程服务器）进行通信以连接全渠道服务端点。 *AEM作者*&#x200B;使用AEM的内置内容编辑器在应用程序中创建丰富的相关体验，包括与Adobe Experience Cloud其余部分的集成。
...
>[!NOTE]
>
>*AEM开发人员的*&#x200B;角色并非以模板和组件的开发开始和结束。 *AEM开发人员*&#x200B;可以创建全新的应用程序，而不是简单地扩展现成的引用实现示例。
