...
Content Services为常见对象（如资产、资产收藏集、HTML页面、应用程序配置和独立于渠道的页面）提供了一些现成的模型。 它们可以配置，因此可满足特定客户需求而无需AEM开发工作。
...
如果现有AEM内容，则使用模型和AEM内容源生成实体。 例如，页面实体是从AEM页面和页面模型生成的独立于渠道和布局的对象。
...
>[!NOTE]
>
>如果模型未与现有AEM内容相对应，例如客户创建了模型，则会显示UI，以便客户创建实体。
>
...
在AEM方面，空间是一种管理相关实体的便捷方式。 它也可用于分配授权权限。 可以对空间进行授权，从而保护该空间中的实体。
