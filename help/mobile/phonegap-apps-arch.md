---
title: 应用程序的剖析
description: 本页介绍了为应用程序创建的基于/libs/mobileapps/components/angular/ng-page组件（本地服务器上的CRXDE Lite）的页面组件。
contentOwner: User
...
根据是否检测到wcmMode (!=WCMMode.DISABLED)，Angular页面正文的呈现方式不同，这将确定打开页面进行创作还是作为已发布的页面。

**创作模式**

在创作模式下，每个页面将单独呈现。 Angular不处理页面之间的路由，也不会使用ng-view加载包含页面组件的部分模板。 相反，页面模板(template.jsp)的内容通过`cq:include`标记包含在服务器端。

此策略可让作者功能（例如，在段落系统、Sidekick、设计模式等中添加和编辑组件）在不进行修改的情况下正常工作。 依赖客户端渲染的页面（例如应用程序的页面）在AEM创作模式下性能不佳。
...
在发布模式下（例如，使用内容同步导出应用程序时），所有页面都会变为单页应用程序(SPA)。 （要了解SPA，请使用Angular教程，尤其是[https://docs.angularjs.org/tutorial/step_07](https://docs.angularjs.org/tutorial/step_07)。）
...
Angular Route服务使用此元素来显示应用程序中每个页面的内容，包括当前页面的可创作内容（包含在template.jsp中）。

body.jsp文件包括空的header.jsp和footer.jsp。 如果您要在每个页面上提供静态内容，则可以在应用程序中覆盖这些脚本。

最后，javascript clientlibs包含在&lt;body>元素的底部，其中包括在服务器上生成的两个特殊JS文件： *&lt;页面名称>*.angular-app-module.js和&#x200B;*&lt;页面名称>*.angular-app-controllers.js。
...
最后，此模块循环访问每个下级页面（包括本身）并呈现每个页面的路由片段的内容（通过angular-route-fragment.js选择器和扩展），包括它作为Angular $routeProvider的配置条目。 换句话说，$routeProvider会告知应用程序在请求给定路径时要呈现的内容。
...
在Angular中，控制器连接$scope中的变量，使它们公开给视图。 angular-app-controllers.js.jsp脚本遵循angular-app-module.js.jsp所示的模式，即遍历每个下级页面（包括本身），并输出每个页面定义的控制器片段（通过controller.js.jsp）。 它定义的模块名为`cqAppControllers`，必须作为顶级应用模块的依赖项列出，以便可以使用页面控制器。
...
此脚本仅输出顶级Angular应用程序模块的Angular依赖项。 angular-app-module.js.jsp引用了该函数。
...
应用程序组件不仅必须在AEM实例（发布或创作）上运行，而且还必须在应用程序内容通过Content Sync导出到文件系统时运行。 因此，该组件必须包括以下特性：
...
为了解决此问题，PhoneGap应用程序使用单页应用程序(SPA)模式，以便基本URI（不包括哈希）永不更改。 因此，您引用的每个资源、模板或脚本**都必须相对于您的顶级页面。 **顶级页面通过`*<name>*.angular-app-module.js`和`*<name>*.angular-app-controllers.js`初始化Angular路由和控制器。 此页面应该是*不*扩展sling:redirect的最接近存储库根的页面。
...
或者，如果`$scope.wcmMode == false`，则每个导航事件都会导致URL的哈希部分发生更改，该更改由Angular的ngRoute模块在内部解析。
...
在由JSON数据驱动的组件（如“ng-text”：/libs/mobileapps/components/angular/ng-text）中，overhead.jsp可用于从template.jsp中删除所有Java代码。 然后，它从template.jsp中引用，并且在请求中公开的任何变量都可供使用。 此策略鼓励逻辑与呈现分离，并限制从现有组件派生新组件时必须复制和粘贴的代码量。

#### controller.js.jsp {#controller-js-jsp-1}

如AEM页面模板中所述，每个组件都可以输出一个JavaScript片段以使用`data`承诺公开的JSON内容。 按照Angular约定，控制器只应用于将变量分配给作用域。

#### angular.json.jsp {#angular-json-jsp}

此脚本作为片段包含在全页“&lt;page-name>.angular.json”文件中，该文件将针对每个扩展ng-page的页面导出该文件。 在此文件中，组件开发人员可以公开组件所需的任何JSON结构。 在“ng-text”示例中，此结构仅包括组件的文本内容，以及一个指示组件是否包含富文本的标记。
...
after-platform_add目录包含`copy_AMS_Conifg.js`文件。 此脚本可复制配置文件以支持Adobe Mobile Services分析的收集。
...
www目录包含实施应用程序外观和行为的所有Web内容（HTML、JS和CSS文件）。 除下述例外情况外，这些内容源自AEM，并通过Content Sync导出为其静态形式。
...
按照惯例，在AEM中，/etc节点包含静态clientlib内容。 etc目录包含Topcoat、AngularJS和Geometrixx ng-clientlibsall库。
...
了解应用程序的剖析后，请参阅[单页应用程序](/help/mobile/phonegap-single-page-applications.md)。
