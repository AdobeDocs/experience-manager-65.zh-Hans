...
每个供应商应用程序商店（例如，Apple App Store或Google Play Store）都需要移动设备应用程序的一个或多个屏幕截图，才能向客户显示您的应用程序详细信息。 这些屏幕截图可能对维度和内容有严格的要求（基本上它们必须真正代表应用程序）。 AEM应用程序支持为支持的平台选择和管理这些屏幕截图，并根据每个供应商的应用程序商店的要求查看端口维度。

>[!NOTE]
>
>AEM Verify应用程序允许您在AEM中将屏幕截图直接发送到应用程序详细信息。
>
>有关更多详细信息，请参阅[AEM移动快速入门验证](/help/mobile/phonegap-mobile-quickstart.md)。
...
#### ™元数据 {#android-metadata}

在选择™平台时，目前没有可设置的自定义元数据。 单击下载按钮后，将生成一个zip文件，其中属性文件包含所有元数据和关联的屏幕截图。
...
AEM应用程序的一项重要功能是，移动应用程序可以通过ContentSync请求新内容，其中内容可以是html资源、页面、视频、图像、文本等。 内容作者更新内容并发布该内容后，服务器将内容更新提供给移动设备应用程序下载。
...
>[!NOTE]
>
>如果您的创作服务器实例复制到多个发布服务器实例（AEM的通用架构），则每个发布服务器都有相同的更新内容。 原因是更新基于创作实例构建并复制到所有发布实例。 基本上，完全支持负载平衡和故障切换。
