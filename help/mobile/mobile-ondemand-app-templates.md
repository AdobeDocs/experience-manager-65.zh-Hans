...
代码存储库中包含一个简单的组件，其源可以从AEM中找到。 随后，也可以在CRXDE Lite中本地打开它。
...
但是，如果没有在AEM Mobile On-demand Services中呈现的自定义导出内容同步处理程序，自定义组件可能无法正确导出并上传到AEM。

一旦组件已包含在AEM页面中，并且已包含其他一些构建基块组件，您就可以将另一个组件添加到该页面或编辑现有组件。
...
>[!NOTE]
>
>您可以在AEM中创建组件，并使用[使用CRXDE Lite进行开发](/help/sites-developing/developing-with-crxde-lite.md)自定义该组件。 根据要求自定义现有组件后，可以使用&#x200B;**管理文章**&#x200B;下的&#x200B;**编辑**&#x200B;选项将其添加到页面中，如上图所示。
