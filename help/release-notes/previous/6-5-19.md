...
* **[AEM Forms Designer中的辅助功能改进](https://helpx.adobe.com/cn/content/dam/help/en/experience-manager/6-5/forms/pdf/using-designer.pdf)**： AEM Forms Designer引入了以下辅助功能改进：

   * AEM Forms Designer现在支持通过新的`<TOC>`和`<TOCI>`标记访问目录，从而允许作者通过“辅助功能”选项卡将这些角色分配给子表单。 该功能支持多级目录结构、带标签的条目、编号引用和跨域子结构，使屏幕阅读器能够正确识别和导航目录内容。

   * AEM Forms Designer现在支持为表单元格配置辅助功能属性，包括范围、行范围和列范围，从而使屏幕阅读器和辅助设备能够正确解释生成的PDF中的表结构。 作者可以直接从“单元格属性”对话框中设置这些属性，并在XML Source视图中验证它们。 配置的属性会准确反映在标记的PDF输出中，并且可以使用Acrobat表编辑器进行验证。
...
* 即使在PDF中标记是正确的，标记在Forms Designer中的排序也会随机更改。(LC-3921313) 
...
* JEE 上的 AEM Forms 安装程序不支持使用 JDK 11.0.20 进行安装。 仅支持使用 JDK 11.0.19 或更早版本安装 JEE 上的 AEM Forms 安装程序。 （FORMS-10659）
