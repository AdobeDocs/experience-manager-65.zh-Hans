...
## [!DNL Experience Manager] 6.5.17.0中包含的内容 {#what-is-included-in-aem-6517}

[!DNL Experience Manager] 6.5.17.0包括自2019年4月首次推出6.5以来发布的新功能、关键客户请求的增强功能、错误修复以及性能、稳定性和安全性改进。 在[!DNL Experience Manager] 6.5上[安装此Service Pack](#install)。
...
* 内存不足；由于`NotificationManagerImpl`，垃圾回收活动率高。 `NotificationManager`包升级到Experience Manager6.5。 (SITES-11440)
...
* Experience ManagerDesigner创建的PDF表单中所有后台对象的PAC 2021工具返回错误为`Path object not tagged`。 (FORMS-6707)
* 当用户在收件箱中应用过滤器时，会引发`NullPointerException`错误。 (FORMS-6706)
* 当用户导入包含引用的片段的模板(.tds)文件时，Experience ManagerDesigner崩溃。 (FORMS-6702)
...
* 当用户尝试使用输出服务生成PDF时，即使它标记为`PDF/A-1a`，使用`Preflight`服务的符合性检查也会失败。 (LC-3920837)
* 安装Experience Manager6.5.16.0 Service Pack后，Experience ManagerDesigner无法打开。 (LC-3921000)
...
* 升级到Experience Manager6.5.14.0 Service Pack后，Workbench无法处理CRX节点从一个位置移动到另一个位置的操作。 此错误以`ALC-CRX-30000-000: com.adobe.ep.crx.client.exceptions.CRCException: ALC-CRX-030-000-[Internal Server Error]`的形式出现。 (FORMS-7713)
* 当用户更新到Experience Manager6.5.16.0 Service Pack时，`Usage Rights`无法应用。 (FORMS-7892)
* 当用户尝试生成PDF文档时，PDF/A-1b验证失败。 (FORMS-7615)
* 当用户单击`Form Container`组件的`Configure`选项时，浏览器将变得无响应。 (FORMS-7605)
...
* 正在向`fd-cloudservice`系统用户添加`modifyProperties`权限以配置Adobe Sign。 (FORMS-6164)
...
* 为虚拟路径提供Sling `ResourceMerger`时会占用大量CPU，从而导致拒绝服务。 (NPR-40338)
...
* 上传具有大型`DocumentAncestors`Experience Manager属性的PSD/PDF时，元数据作者崩溃。 (ASSETS-22966)
* 包含`user-reader-service`的类`InboxSharingCache`中的会话泄漏。 (CQ-4352513)
...
有两种不同的方法可用于自动安装[!DNL Experience Manager] 6.5.17.0.<!-- UPDATE FOR EACH NEW RELEASE -->
...
1. 产品信息页面(`/system/console/productinfo`)在[!UICONTROL 已安装的产品]下显示更新的版本字符串`Adobe Experience Manager (6.5.17.0)`。<!-- UPDATE FOR EACH NEW RELEASE -->
...
* [Experience Manager6.5.17.0](/help/release-notes/assets/65170_bundles.txt)中包含的OSGi包列表<!-- UPDATE FOR EACH NEW RELEASE -->
* [Experience Manager6.5.17.0](/help/release-notes/assets/65170_packages.txt)中包含的内容包列表<!-- UPDATE FOR EACH NEW RELEASE -->
