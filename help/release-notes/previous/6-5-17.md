...
[!DNL Experience Manager] 6.5.17.0包括自2019年4月首次推出6.5以来发布的新功能、关键客户请求的增强功能、错误修复以及性能、稳定性和安全性改进。 [安装此Service Pack](#install) 日期 [!DNL Experience Manager] 6.5.
...
* 打开非营销活动页面调用 `targeteditor.html` 不必要的。 删除 `targeteditor` 不需要时调用。 (SITES-12469)
* 无法为具有批注的页面创建活动副本。 (SITES-12154)
* 页面转出在Experience Manager6.5.16上不起作用。 (SITES-12008)
* 内存不足；由于以下原因，垃圾回收活动率高 `NotificationManagerImpl`. `NotificationManager` 捆绑包升级到Experience Manager6.5。 (SITES-11440)
...
* 使用 `SemanticDataType.REFERENCE` 应支持“Remote-Asset-IDs”。 (SITES-12127)
* 将Polaris资产选择器集成到内容片段编辑器中。 (SITES-12125)
* 访问元数据服务端点需要强制的http标头。 (SITES-13068)
* GraphQL 6.5的实施与Cloud Service（主要）不符；已确定的问题已得到修复。 (SITES-13096)
* GraphQL分页/排序和混合筛选应该在Experience Manager6.5/AMS上可用。 (SITES-9154)
...
* 在Experience Manager内容查找器侧边栏中设置的轮播不会显示任何缩略图。 (SITES-8593)
...
* 使用编辑资源时 `Desktop Actions`，如果您尝试同时签入五个以上的资产， `Limit Reached` 此时会显示错误并签出选定的资产。 (ASSETS-21121)
* 无法按收藏集中的名称对资源排序。 (ASSETS-20924)
* 无法在图像格式类型的资产上设置维度。 (ASSETS-20835)
* 共享链接时，“搜索/添加电子邮件地址”字段上的工具提示文本及其背景不显示相应的对比度。 (ASSETS-17347)
* 当您展开时 `Notifications`时，由于段落间距，文本无法正常显示。 (ASSETS-17345)
* 在收藏集中复制资产时， `Public Collection` 复选框未正确显示。 (ASSETS-17343)
* 元素在没有角色的情况下使用ARIA属性。 (ASSETS-17325，ASSETS-17323)
* 展开时，链接不是描述性的 `Notifications`. (ASSETS-17283)
* 当您导航并展开 [!DNL Smart Crop] 按钮时，内容看起来像列表一样，但未标记为未排序列表。 因此，屏幕阅读器无法识别无序列表并将其阅读为纯文本。 (ASSETS-17247)
* 此 `Sort By` 标签不与其相应的下拉列表关联。 因此，屏幕阅读器无法识别下拉选项。 (ASSETS-17239)
* 尝试使用添加用户时，无法使用键盘Tab键或箭头键向前或向后移动 `Add user` 组合框。 (ASSETS-17233)
* 屏幕阅读器无法正确传达工作流步骤的信息(ASSETS-17285)。
* 当您导航到 `Saved Searches` 组合框内，名称和角色均没有任何分配的标签。 (ASSETS-17329)
...
* 提高了浏览包含大量Dynamic Media视频的文件夹时的性能，并解决了在文件夹信息卡视图上加载失败的问题。 (ASSETS-23016)
...
* 在Experience Manager上预览Dynamic Media文件时，现在会遵循Experience ManagerACL（访问控制列表）。 (ASSETS-21628)
...
* 在更新到AEM 6.5.15.0 Service Pack后，HTML5表单在具有IE兼容模式的Edge浏览器中无法正常工作或加载。 (FORMS-8526、FORMS-8523)
...
* 在 `Assign task` 步骤，当用户选择“发送通知电子邮件”并调用工作流时，文本未正确显示在收到的电子邮件中。 接收问号，而不是接收的电子邮件中的文本。 (FORMS-7675)
...
* 为了提高Experience Manager6.5.16.0 Service Pack上的PDFG转换性能，可以配置休眠间隔。 (FORMS-6752)
...
* 对于由Experience Manager设计者创建的PDF表单中的所有背景对象，PAC 2021工具返回错误为 `Path object not tagged`. (FORMS-6707)
* 当用户在收件箱中应用过滤器时，会抛出 `NullPointerException` 错误。 (FORMS-6706)
* 当用户导入包含引用的片段的模板(.tds)文件时，Experience Manager设计器崩溃。 (FORMS-6702)
...
* 当用户创建简单工作流并添加简单变量时， `set variable mapping` 发生错误。 (FORMS-5819)
* 当用户尝试使用输出服务生成PDF时，即使它标记为 `PDF/A-1a`，使用进行的合规性检查`Preflight` 服务失败。 (LC-3920837)
* 安装Experience Manager6.5.16.0 Service Pack后，Experience Manager设计器无法打开。 (LC-3921000)
* 当用户添加复选框和单选按钮时，不会根据PDF标准生成Tag树的结构。 (LC-3920838)
* 如果用户通过输出服务使用字体的嵌入和子设置来生成静态PDF，则生成的PDF仅包含嵌入的字体。 (LC-3920963)
* 希伯来文本以RTL格式显示不正确。 (LC-3919632)
* 当用户在JBoss® Turnkey服务器上升级到Experience Manager6.5.16.0 Service Pack时，签名服务无法调用。 遇到的错误是： `java.lang.ClassCastException: com.adobe.xfa.TextNode cannot be cast to com.adobe.xfa.Element`. (FORMS-7833)
...
* 当用户更新到Experience Manager Forms 6.5.16.0 Service Pack并尝试更改 `LicenseType` 到 `Production`，则不会反映更改。 (FORMS-7594)
* 当用户尝试使用包含以下PDF的LCA进程时 `Chinese Full Width Characters`，的问题 `ValidateForm` 进程。 (FORMS-7464)
* 在Experience Manager Forms Designer中，XMLFM为基于XDP的模板生成具有不同纸张大小的ZPL输出，例如信纸、A4和A5。 (FORMS-7898)

### [!DNL Commerce]{#commerce-6517}

* 已移动的标记属于垃圾收集，但仍被下的产品引用。 `/var`. (CQ-4351337)

### Foundation{#foundation-6517}

#### 集成{#integrations-6517}

* 在旧版云配置中，将Adobe Target IMS配置转换为用户凭据时， `connectedWhen` 属性不会更改。 此问题使得所有调用看起来就像配置仍然基于IMS一样。 (CQ-4352810)
* 正在添加 `modifyProperties` 权限 `fd-cloudservice` Adobe Sign配置的系统用户。 (FORMS-6164)
* 通过与Adobe Target集成的Experience Manager，在创建AB测试活动时，不会将与其关联的受众同步到Target。 (NPR-40085)
* 消除有关类的消息 `GuavaDeprecation`，您可以更改日志级别。 有关详细信息，请访问 [https://experienceleaguecommunities.adobe.com/t5/adobe-experience-manager/how-to-get-rid-of-messages-about-guava-deprecation/td-p/606978](https://experienceleaguecommunities.adobe.com/t5/adobe-experience-manager/how-to-get-rid-of-messages-about-guava-deprecation/td-p/606978).
...
* 在Experience ManagerTag Management用户界面(/aem/tags/)中，命名空间和标记将按其创建顺序显示。 但是，当存在许多命名空间和标记时，查看和管理它们的功能就会变得困难。 这个问题是因为它们不能以任何其他方式排序。 (NPR-39620)
...
* 工作流控制台更改。 (NPR-40502)
* `SegmentNotfound errors` 在生产创作实例的日志中，由类中的资源解析程序未关闭导致 `com.day.cq.workflow.impl.email.EMailNotificationServic`. (NPR-40187)
* 已关闭的未关闭 `ResourceResolver` 正在记录异常。 (ASSETS-22495)
* Experience Manager创作在PSD/PDF大量内容时崩溃 `DocumentAncestors` 元数据属性已上载。 (ASSETS-22966)
* 类中的会话泄露 `InboxSharingCache` 替换为 `user-reader-service`. (CQ-4352513)
...
* 在具有MongoDB和多个实例的部署中，安装 [!DNL Experience Manager] 6.5.17.0适用于使用包管理器的某个Author实例。<!-- UPDATE FOR EACH NEW RELEASE -->

>[!IMPORTANT]
>
> Adobe不建议您删除或卸载 [!DNL Experience Manager] 6.5.17.0程序包。 因此，在安装该包之前，您应该创建 `crx-repository` 万一你一定要把它退回。 <!-- UPDATE FOR EACH NEW RELEASE -->
