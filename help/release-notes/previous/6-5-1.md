...
[!DNL Adobe Experience Manager] 6.5.1.0是一个重要版本，它包括自正式发布以来所推出的性能、稳定性、安全性和关键客户修复及增强功能。 [!DNL Adobe Experience Manager] 6.5英寸 *2019年4月。* 它可以安装在顶部 [!DNL Experience Manager] 6.5.

此Service Pack版本的一些关键亮点包括：

* 允许在跟踪事件中包含dynamic-UI-state作为自定义属性。
* 包括支持在中交付360度视频资产 [!DNL Dynamic Media]-Scene7模式。
* 已启用 *日语自动换行* 通过富文本编辑器的样式插件实现的功能。 有关更多信息，请参阅 [配置日文自动换行](/help/sites-administering/configure-rich-text-editor-plug-ins.md#jpwordwrap)

## 资产

* 针对 S3 多部分支持更新了 DAM DMGateway 接口。NPR-29740：适用于 CQ-4226303 的修补程序
* 生成演绎版预览 `Only empty tenantId is currently supported` 升级到 [!DNL Experience Manager] 6.5. NPR-29986：适用于CQ-4272353的修补程序
...
* 为启用了多站点管理器支持 [!DNL Experience Manager Assets]. 适用于CQ-4271453、CQ-4268621、CQ-4257491的修补程序
* [!DNL Experience Manager] 界面应在时间轴历史记录中显示资产当前版本的附加条目，并显示来自的最新签入注释 [!DNL Adobe Asset Link]. 适用于 CQ-4262864 的修补程序
...
* CoralUI，与配合使用时 `Multifield`，存储 `fileReferenceParameter` 组件级别而非多字段级别。 NPR-29537：适用于 CQ-4266129 的修补程序
...
* 将社交帖子设置到的时出现错误消息 [!DNL Facebook] 身份验证。 NPR-29211：适用于 CQ-4266630 的修补程序
...
* 打开包含美元字符的内容片段时出现分析错误 `($)` 和左大括号 `({)`. 适用于 CQ-4270266 的修补程序
...
* 用户提供的数据不会转义到中的输出 `cq/replication/components/agent` 组件，从而导致存储型跨站点脚本(XSS)漏洞。 适用于 CQ-4266263 的修补程序
...
* 尝试编辑和保存时，会显示异常消息 [!DNL Experience Manager] 启动配置。 NPR-29176：适用于CQ-4265782/CQ-4266153的修补程序
...
* 复制代理组件容易受到漏洞攻击，该漏洞会向未经授权的用户泄露敏感信息。NPR-29611：适用于GRANITE-25070的修补程序

* 在OAuth期间，对的每个复制的会话泄露 [!DNL Brand Portal]. NPR-30001：适用于GRANITE-26196的修补程序

## 项目

* Publish [!DNL Experience Manager Assets] 从 [!DNL Experience Manager] 将/content/dam/mac文件夹创作到 [!DNL Brand Portal] 不起作用。 NPR-29819：适用于 CQ-4271118 的修补程序

## Platform

* HtmlLibraryManager在缓存失效时删除crx-quickstart的所有内容。 NPR-29863：适用于GRANITE-26197的修补程序
...
的主要功能亮点 [!DNL Experience Manager Forms] 6.5.1.0包括：
...
* 在中恢复了自适应Forms的验证步骤 [!DNL Experience Manager] 6.5.适用于CQ-4269583的修补程序

* 自适应表单字段验证失败中断 [!DNL Adobe Sign]. 适用于 CQ-4269463 的修补程序
* 当 [!DNL Experience Manager Forms] 实例有超过20个自适应表单片段，所有表单片段的名称都以同一字符串开头，搜索不会返回或只返回最近创建的20个片段。 适用于 CQ-4264414、CQ-4264914 的修补程序
...
* [!DNL Experience Manager Forms] 6.5创建通信UI (CCR UI)无法打开与创建的通信 [!DNL Experience Manager Forms] 6.3.适用于CQ-4266392的修补程序
...
* 在使用创建的静态PDF上执行完整的辅助功能检查时 [!DNL Experience Manager Forms Designer]中，由于缺少语言属性，主语言检查失败。 适用于 CQ-4272923、CQ-4271002 的修补程序
...
* 创建的静态PDF [!DNL Experience Manager Forms] 服务器的输出模块不会使用所创建文档的语言填充语言属性/标记。 NPR-27332：适用于 CQ-4271002 的修补程序
...
* 为启用了多站点管理器支持 [!DNL Experience Manager Assets]. 有关更多信息，请参阅 [使用MSM for Experience Manager Assets重用资源](https://experienceleague.adobe.com/docs/experience-manager-65/assets/using/reuse-assets-using-msm.html). NPR-29199：适用于 CQ-4259922 的修补程序
