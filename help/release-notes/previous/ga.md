...
* 运行AEM 6.2、6.3和6.4的客户支持直接就地升级到AEM 6.5。 使用5.x或6.0/6.1且希望就地升级的客户，需要先升级到6.4。 然后再升级至 6.5，或者也可以通过在实例之间直接传输内容的方式升级至 AEM 6.5。
* 在 6.5 中，升级流程基本保持不变。
...
**@Mentions支持**
AEM Communities现在允许注册用户在用户生成的内容中标记（提及）其他注册成员以吸引他们的注意。 被标记（提及）的成员会收到通知，并带有指向相应用户生成内容的深度链接。 但是，用户可选择启用或禁用 Web 和电子邮件通知。
...
[!DNL Adobe Experience Manager]提供了多种方法来与[!DNL Adobe Creative Cloud]集成并共享资产，以便在创意和营销团队或业务团队密切协作的工作流中使用。[!DNL Experience Manager] 6.5在集成方面继续改进，并进一步精简，以发掘更多机会并简化现有方法。
