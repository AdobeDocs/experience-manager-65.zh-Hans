...
* 当用户从使用® WebSphere®部署的AEM Forms应用AEM Forms Service Pack 14 (SP14)时，初始化数据库时引导失败，并出现`java.lang.NoClassDefFoundError:org/apache/log4j/Logger`错误。(NPR-39414)
...
* 在AEM Designer中，不带标题的表单字段会放置在包含复选框的表单元格中。(LC-3920410)
...
* 在列表视图中按升序和降序对标题进行排序，了解标题的顺序A、C、B。 (SITES-7585)
...
1. OSGi捆绑包`org.apache.jackrabbit.oak-core`的版本为1.22.13或更高版本（使用Web控制台： `/system/console/bundles`）。
