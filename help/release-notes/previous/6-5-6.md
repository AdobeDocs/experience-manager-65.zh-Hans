...
* 支持 [automated forms conversion服务中的Adobe Sign文本标记](https://experienceleague.adobe.com/docs/aem-forms-automated-conversion-service/using/convert-existing-forms-to-adaptive-forms.html).
...
* 在下创建品牌 `/content/campaign` 生成不允许您创建营销活动的结构。 [!UICONTROL 创建品牌] 选项使新创建的品牌无法创建 [!UICONTROL 优惠和活动] 因为没有 [!UICONTROL 创建] 选项(NPR-34113)。
