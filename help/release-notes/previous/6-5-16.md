---
title: ' [!DNL Adobe Experience Manager] 6.5.16.0的发行说明'
...
## [!DNL Experience Manager] 6.5.16.0中包含的内容 {#what-is-included-in-aem-6516}

[!DNL Experience Manager] 6.5.16.0包括自2019年4月首次推出6.5以来发布的新功能、关键客户请求的增强功能、错误修复以及性能、稳定性和安全性改进。 [在[!DNL Experience Manager] 6.5上安装此Service Pack](#install)。<!-- UPDATE FOR EACH NEW RELEASE -->
...
* `dam:size`属性在XMP写回后未更新，这会导致从`/platform/path/to/asset.jpg;resource=metadata` API返回的信息不正确。 (ASSETS-17631)
...
* 由于JCR属性读取为`toString`而不是`getString`，`getManifestURI`返回null。 (ASSETS-18674)
...
* 在Dynamic Media“常规设置”页面中，会显示未翻译的错误消息`Failed to fetch data`。 (ASSETS-15617)
...
安装AEM 6.5.16.0 Forms附加组件服务包后，客户面临以下列出问题。 因此，已发布[AEM 6.5.16.0 Forms附加组件Service Pack - 6.0.914](https://experienceleague.adobe.com/docs/experience-manager-release-information/aem-release-updates/forms-updates/aem-forms-releases.html)的更新版本。 Adobe建议使用更新的Service Pack：
...
* 从Experience Manager6.5中删除AdobeSearch&amp;Promote代码和依赖关系。AdobeSearch&amp;Promote已于2022年9月终止服务。 请参阅[AdobeSearch&amp;Promote服务结束公告](https://experienceleague.adobe.com/docs/discontinued/using/search-promote.html)。 (NPR-39706)
...
有两种不同的方法可用于自动安装[!DNL Experience Manager] 6.5.16.0.<!-- UPDATE FOR EACH NEW RELEASE -->
...
1. 产品信息页面(`/system/console/productinfo`)在[!UICONTROL 已安装的产品]下显示更新的版本字符串`Adobe Experience Manager (6.5.16.0)`。<!-- UPDATE FOR EACH NEW RELEASE -->
...
使用GraphQL的客户应安装包含GraphQL索引包1.0.5](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=%2Fcontent%2Fsoftware-distribution%2Fen%2Fdetails.html%2Fcontent%2Fdam%2Faem%2Fpublic%2Fadobe%2Fpackages%2Fcq650%2Ffeaturepack%2Fcfm-graphql-index-def-1.0.5.zip)的[AEM内容片段。
...
* [Experience Manager6.5.16.0](/help/release-notes/assets/65160_bundles.txt)中包含的OSGi包列表<!-- UPDATE FOR EACH NEW RELEASE -->
* [Experience Manager6.5.16.0](/help/release-notes/assets/65160_packages.txt)中包含的内容包列表<!-- UPDATE FOR EACH NEW RELEASE -->
