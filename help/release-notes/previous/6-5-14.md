...
* **[AEM Forms Designer中的列表辅助功能支持](https://helpx.adobe.com/cn/content/dam/help/en/experience-manager/6-5/forms/pdf/using-designer.pdf)**： AEM Forms Designer现在允许作者通过辅助功能选项卡将列表、列表项和列表标签角色分配给子表单，以创建无障碍的列表结构。 该功能支持简单、编号、项目符号和嵌套列表，通过适当的`<L>`、`<LI>`、`<Lbl>`和`<Lbody>`标记生成，以实现屏幕阅读器兼容性。 此增强功能可确保表单列表符合PDF/UA无障碍标准，从而改进辅助技术用户的导航和内容理解能力。
...
* 增加了在Forms Designer中为超链接指定屏幕Reader文本的功能。(NPR-36221)
