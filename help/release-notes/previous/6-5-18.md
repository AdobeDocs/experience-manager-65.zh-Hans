...
* 如 [配置浏览器文档](https://experienceleague.adobe.com/docs/experience-manager-65/administering/introduction/configurations.html#using-configuration-browser)， _名称将成为存储库中的节点名称_. 但是，在Configuration Browser中， Configuration Title用于CRXDE Lite中的路径，而Name of the Configuration被忽略。 (NPR-40607)
...
* **升级到Service Pack 18 (6.5.18.0)后，页面发布在页面编辑器中不起作用**
  ...
  将AEM 6.5.0.0 - 6.5.17.0的实例升级到AEM 6.5.18.0后，单击 **发布页面** 在页面编辑器中，您被重定向到一个不存在的URL。
