...
* 当用户使用嵌入容器将自适应表单（AF）嵌入到网页（无论是外部网页还是 AEM Sites 页面）时，自适应表单引导容器会引入一个 ARIA LABEL。该标签为嵌入表单设置了 role=&quot;main&quot;。根据 ARIA 指南，每个页面只能有一个 role=&quot;main&quot;。因此，当用户为页面的主要内容添加另一个 role=&quot;main&quot; 时，就会被标记为辅助功能问题。（FORMS-13538）
...
>[!IMPORTANT]
>
> Adobe 不建议移除或卸载 [!DNL Experience Manager] 6.5.21.0 包。因此，在安装该软件包之前，您应创建 `crx-repository` 的备份，以便在需要时进行回滚。<!-- UPDATE FOR EACH NEW RELEASE -->
...
AEM 6.5.21、6.5.22、6.5.23 和 AEM 6.5 LTS GA 随附的 `org.apache.sling.scripting.jsp:2.6.0` 捆绑包存在已知问题。该问题通常在高负载情况下出现，即当 AEM 实例处理大量并发请求时发生。
...
* 如果用户尝试使用已保存的 XML 数据预览草稿信件，某些特定信件会卡在 `Loading` 状态。要下载并安装热修复补丁，请参阅 [Adobe Experience Manager Forms 修补程序](/help/release-notes/aem-forms-hotfix.md#hotfix-for-adaptive-forms)文章。（FORMS-14521）
...
这些网站仅向客户开放。如果您是客户并需要访问权限，请联系您的 Adobe 客户经理。
