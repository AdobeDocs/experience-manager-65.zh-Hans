...
Experience Manager 6.5.25.0包括新增功能、客户请求的关键增强功能和错误修复。 此外，还包括基于2019年4月以来推出的6.5基础在性能、稳定性和安全性方面做出的改进。

此Service Pack版本提供了Sites、Assets和Foundation的275个后端口，包括关键错误修复和安全强化。 此版本还通过大量键盘导航、焦点管理、ARIA语义、颜色对比度改进和触屏目标调整等与WCAG标准相一致的设置，提高了站点创作过程的辅助功能。

此版本中默认提供交叉窗口，无需在安装后单独安装软件包或配置。

安全后端口可解决XSS漏洞并改进共享资产元数据的处理。

内容片段和GraphQL API也获得了可靠性改进，涵盖了嵌入的图像引用、持久查询处理和编辑器本地化。
...
* [基于XFA的PDF的分层书签](https://helpx.adobe.com/cn/content/dam/help/en/experience-manager/6-5/forms/pdf/using-designer.pdf)：输出服务和AEM Forms Designer现在在静态交互式和基于XFA的平面PDF中生成结构化书签层次结构。 书签将遵循在文本框的“辅助功能”属性中设置的标题级别(H1-H6)，因此H2-H6条目将嵌套在正确的父项下，而不是并行显示。
...
* 页面编辑器中的“启动工作流”对话框现在显示本地化的工作流操作标签。 对于工作流选项，作者不会再看到英文字符串。 选项包括批准、发布、请求和取消发布操作。 (SITES-18103)
...
* 现在，交互式通信预览功能会在AEM Forms Service Pack 6.5.24.0之后正确加载内容。 文本不再加载缓慢，缺少空格，因此预览与创作的内容匹配，并且仍然更易于阅读。 (FORMS-25346)
* 配置验证模式并保存表单后，验证详细信息现在显示在自适应Forms核心组件中。 该图案在创作界面中保持可见。 (FORMS-25236)
* 文档生成现在可以在AEM Forms 6.5 Service Pack 23和AEM Forms Service Pack 6.5.24.0环境中正确处理嵌套的可扩展Forms描述语言(XDP)片段。 (FORMS-25234)
* 在使用Rhino引擎进行自适应Forms服务器端重新验证期间，表单片段外部的本地化文本现在以正确的语言显示。 文本在提交后不再恢复到默认语言。 (FORMS-25224)
...
* HTML到可移植文档格式(PDF)转换在AEM Forms Service Pack 6.5.24.0中不再间歇性超时，包括较大的或复杂的转换作业。 (FORMS-23978)
* 安装修补程序AEMForms-6.5.0-0112后，在管理员用户界面中管理安全备份模式现在可以正常工作。 当选项可用时，用户可以禁用或关闭安全备份模式。 （FORMS-23976和FORMS-23718）
* 表单数据模型数据源搜索不再失败或在搜索结果中显示原始HyperText标记语言(HTML)标记。 结果将显示可读文本。 (FORMS-23875)
* 现在，当使用AEM Forms Service Pack 6.5.24.0中支持的核心组件版本在站点页面中嵌入自适应Forms时，运行时加载自定义函数。 (FORMS-23802)
* 现在，当使用更新的Sling Commons JavaScript对象表示法(JSON)库进行解析时，交互式通信数值规则可正确评估值。 数值字段以一致的方式处理BigDecimal解析更改。 (FORMS-23733)
* 现在，在版本6.5.0及更高版本中工作时，记录文档的行为是一致的。 表单输出和相关处理符合新环境的预期行为。 (FORMS-23338)

#### Forms JEE {#forms-jee-6525}

* 加载日志记录组件时，如果找不到org.owasp.esapi.reference.JavaLogFactory错误的类，则应用程序启动不再失败。 受影响的环境无需重新启动服务或重新配置即可正确初始化。 (FORMS-25348)
* 应用Service Pack 25后启动成功完成。 在初始化完成之前，系统引导过程不再失败。 (FORMS-25347)
* Core Portable Document Format (PDF) (CPDF)组件现在可以在Linux环境中正确构建和运行，而不会在执行期间意外停止。 (FORMS-25115)
* 在条形码提取期间处理某些可移植文档格式(PDF)文件时，条形码解码不再因DecodingException错误而失败。 （FORMS-23534和FORMS-23449）

#### Forms Designer {#forms-designer-6525}

* 在6.5版中使用ExportData时，ExportData输出现在与表单数据匹配。 导出的可扩展标记语言(XML)字段与原始表单中显示的值一致。 （LC-3922791）
* 现在，输出服务在AEM Forms Service Pack 6.5.22.0中创建的Portable Document Format (PDF)文件中生成正确的辅助功能标记。 辅助型技术可按正确顺序读取内容，而不会跳过元素。 （LC-3922756）
* 现在，在拼合启用了标记的便携式文档格式(PDF)表单时，输出服务可正确反映隐藏或禁用的字段状态。 （LC-3922708）
* Workbench中具有底部标题的字段现在可接收正确的标记，这可以提高所生成文档的一致性和可访问性。 （LC-3922619）
* 升级到AEM Forms Service Pack 6.5.20.0后，表单上的二维码仍可快速浏览。 标准扫描仪可靠地读取生成的二维码。 （LC-3922551）
* 现在，在不同的Service Pack中使用相同的输入时，Forms Service Render会生成一致的输出。 AEM Forms Service Pack 6.5.17.0和AEM Forms Service Pack 6.5.18.0之间渲染的值不再不同。 （LC-3922461）
* 从XML数据包(XDP)模板生成的PDF/A文档现在可正确呈现下陷方形边框样式。 表单字段外观与设计布局匹配。 （LC-3922180）
* 拼合的可移植文档格式(PDF)提交现在保留所有部分中的数据。 输入的值将保留在最终拼合文档中。 （LC-3922008）
* 转换后的静态可移植文档格式(PDF)文件不再通过原始XDP模板中的单个链接生成多个链接标记。 (LC-3921997)
* 现在，从AEM Forms导出提交内容时，导出输出文件中会包含所有字段。 （LC-3921983）
...
* AEM 6.5 LTS SP1实例升级到LTS SP2后，Adobe CRX包管理器测试套件不再失败。 现在，缩略图列表servlet测试可按预期完成。 (NPR-43534)
...
* **FORMS-23703**&#x200B;如果未使用默认值配置`contains`规则时，自适应表单的服务器端验证失败。 您可以安装最新版本的[AEM Forms 6.5.25.0 Service Pack](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/forms-updates/aem-forms-releases#aem-65-forms-releases)以修复此问题。
* **GRANITE-63681**&#x200B;默认系统配置阻止所需的关键字和正则表达式模式，这会阻止表单数据模型连接器进行身份验证。 要解决此问题，请从[链接](/help/release-notes/aem-forms-hotfix.md)下载并安装修补程序。
...
* **FORMS-23875**&#x200B;在表单数据模型搜索中，即使相关实体不存在，UI也会显示HTML标记。 要解决此问题，请从[链接](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq650/featurepack/bb-expressionmanager-pkg-10.0.48.zip)下载并安装修补程序。
...
* [Experience Manager中包含的OSGi包列表6.5.25.0](/help/release-notes/assets/65250-bundles.zip)
<!-- UPDATE FOR EACH NEW RELEASE -->
* [Experience Manager中包含的内容包列表6.5.25.0](/help/release-notes/assets/65250-packages.zip)
<!-- UPDATE FOR EACH NEW RELEASE -->
