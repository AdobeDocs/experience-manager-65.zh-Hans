...
* Teaser对话框中的确认操作会向屏幕阅读器正确公告。 控件报告`Confirm`不是图标说明，为键盘和屏幕阅读器用户提供明确的指导。 （SITES-25223）
...
* 屏幕阅读器现在会在“注释”工具栏中准确朗读“注释”和“关闭注释”控件。 NVDA不再显示`Preview button pressed,`误导了作者并建议了错误的操作。 公告与按下的按钮相匹配，并保持工作流清晰。 （SITES-25234）
...
* AEM在Teaser模式中宣布加载状态。 该模式现在会在内容加载时显示实时状态消息，因此NVDA和JAWS会说`Loading, please wait.`作者应该收到明确的反馈，并在对话框准备就绪之前避免与对话框交互。 （SITES-25366）
...
* “类别”下拉列表缺少特定标签，因此JAWS表示通用`images button menu.`。更新将命名控件`Categories`并定义其角色。 屏幕阅读器用户可听到准确的标签并了解可用的选择。 （SITES-35226）
