...
1. 以OSGi捆绑包形式安装MySQL数据库的JDBC驱动程序：
   ...
   1. 选择 **安装/更新**. An **上传/安装包** 出现对话框。

   1. 选择 **选择文件** 浏览并选择MySQL JDBC驱动程序OSGi包。 选择 **开始捆绑包** 和 **刷新包**，并选择 **安装** 或 **更新**. 确保Oracle公司的MySQL JDBC驱动程序处于活动状态。 已安装驱动程序。

1. 将MySQL数据库配置为数据源：

   1. 转到位于的AEM Web控制台 [https://localhost:4502/system/console/configMgr](https://localhost:4502/system/console/configMgr).
   1. 定位 **Apache Sling连接池化数据源** 配置。 选择以在编辑模式下打开配置。
   1. 在配置对话框中，指定以下详细信息：
   ...
   将其他属性保留为默认值 [值](https://tomcat.apache.org/tomcat-7.0-doc/jdbc-pool.html) 并选择 **保存**.
...
1. 选择 **创建** > **表单数据模型**.
1. 在创建表单数据模型向导中，指定 **name** 用于表单数据模型。 例如， **FDM_Create_First_IC**. 选择&#x200B;**下一步**。
1. 选择数据源屏幕列出了所有已配置的数据源。 选择 **MySQL** 数据源并选择 **创建**.
...
1. 此 **FDM_Create_First_IC** 此处列出了您之前创建的表单数据模型。 选择它并选择 **编辑**.
...
1. 选中顶部的复选框 **帐单** 要选择的数据模型对象 **创建子属性**.
1. 在 **创建子属性** 窗格：

   1. 输入 **使用费用** 作为子资产的名称。
   1. 启用 **已计算**.
   1. 选择 **浮动** 作为类型并选择 **完成** 将子属性添加到 **帐单** 数据模型对象。

   ![创建子属性](assets/create_child_property_new.png)

1. 选择 **编辑规则** 以打开规则编辑器。
1. 选择&#x200B;**创建**。此 **设置值** 规则窗口将打开。
1. 从选择选项下拉列表中，选择 **数学表达式**.

   ![使用费规则编辑器](assets/usage_charges_rule_editor_new.png)

1. 在数学表达式中，选择 **callcharges** 和 **confcallcharges** 分别作为第一和第二对象。 选择 **加** 作为运算符。 在数学表达式中选择，然后选择 **扩展表达式** 添加 **smscharges**， **网间费用**， **roamingnational**， **roamingintel**、和 **vas** 表达式中的对象。
...
1. 选中顶部的复选框 **客户** 要选择的数据模型对象 **添加关联**. 此 **添加关联** 属性窗格打开。
1. 在 **添加关联** 窗格：
   ...
   * 选择 **添加** 以链接 **客户** 数据模型对象到 **调用** 数据模型对象。 根据用例，调用数据模型对象必须链接到客户数据模型对象中的手机号码属性。 此 **添加参数** 对话框打开。
...
1. 选择 **完成** 在customer和calls数据模型对象之间创建1：n关联。
...
清单和客户数据模型对象分别使用billplan (bills)和customerplan (customer)属性链接。 创建这些属性之间的绑定以检索MySQL数据库中任何可用客户的计划详细信息。
   ...
   * 选择 **完成** 创建billplan和customerplan属性之间的绑定。
...
1. 选中顶部的复选框 **客户** 要选择的数据模型对象 **编辑属性**. 此 **编辑属性** 窗格打开。
1. 指定 **客户** 作为 **顶级模型对象**.
...
1. 选中顶部的复选框 **调用** 要选择的数据模型对象 **编辑属性**. 此 **编辑属性** 窗格打开。
...
1. 转到 **服务** 选项卡。
1. 选择 **get** 服务和选择 **编辑属性**. 此 **编辑属性** 窗格打开。
1. 在 **编辑属性** 窗格：
...
1. 选择 **更新** 服务和选择 **编辑属性**. 此 **编辑属性** 窗格打开。
1. 在 **编辑属性** 窗格：
...
1. 转到 **模型** 选项卡，选择 **客户** 数据模型对象，并选择 **测试模型对象**.
1. 在 **测试表单数据模型** 窗口，选择 **读取模型对象** 从 **选择模型/服务** 下拉列表。
1. 在 **输入** 部分，指定值 **mobilenum** 已配置的MySQL数据库中存在的属性，然后选择 **测试**.
...
1. 在 **编辑示例数据** 窗口，根据需要编辑数据，然后选择 **保存**. 关闭窗口。
