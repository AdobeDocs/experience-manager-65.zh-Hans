...
1. 系统显示消息：“Letter created successfully（已成功创建信件）”。 （在警报消息中）选择 **打开** 以配置其中的数据模块和布局片段。 或选择 **完成** 以返回上一页。

   ![警报消息：书信创建成功](assets/createcorrespondencecreated.png)

   **下一个**：当您选择时 **打开**，通信管理显示布局的表示形式，其中列出了布局(XDP)中的所有组件。 继续插入 [数据模块和布局片段及其配置](/help/forms/using/create-letter.md#p-insert-data-modules-and-layout-fragments-in-a-letter-and-configure-them-p).
...
1. 选择 **插入** 对于每个子表单，选择要插入每个子表单中的数据模块或布局片段。
...
1. 要编辑模块，请选择模块旁边的编辑图标。 编辑模块后，选择 **保存**.
   ...
   1. **允许自由文本**：如果启用了允许自由文本，则用户可以在CCR视图的书信中添加内联文本。 在CCR视图中，为启用了允许自由文本的目标区域启用了“T”操作，当用户选择它时，它会询问文本的名称和描述，然后点按确定时，它会以编辑模式打开该文本，用户可以在其中添加文本。 这与其他文本模块类似
   1. **锁定顺序**：锁定信件中子表单的顺序。 创建信件时，不允许作者对子表单/组件重新排序。

   在此页面中，您还可以对子表单中的每个资产执行以下操作：

   1. **更改资产的顺序**：拖放按住资源重新排序图标的资源( ![拖放](assets/dragndrop.png))。
   1. **删除资源**：选择资产旁边的删除图标以将其删除。
   1. **预览资源**：选择显示预览图标( ![showpreview](assets/showpreview.png))。
...
1. 指定链接后，选择 **下一个**. “通信管理”将显示“附件”屏幕。

### 设置附件 {#set-up-the-attachments}

1. 选择 **添加资源**.
1. 在选择资产屏幕中，选择要与字母一起附加的资产，然后选择 **完成**. 您需要先将资产上传到Assets。 建议您仅附加PDF和Microsoft Office文档，但也可以附加图像。 有关在DAM中上传资产的更多信息，请参阅 [正在上传资产](/help/assets/manage-assets.md).
1. 要锁定列表中的资产顺序，以便理赔人不能更改顺序，请选择 **锁定顺序**. 如果不选择此选项，则索赔理算员可以更改列表项的顺序。
1. 要更改资源的顺序，请拖放一个按住资源重新排序图标的资源( ![拖放](assets/dragndrop.png))。
1. 选择 **编辑** 前面，如果不希望作者能够删除附件，请将附件指定为必选项。 如果要在CCR界面中预先选择附件，请将附件指定为“已选定”。
