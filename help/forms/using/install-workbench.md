...
>[!NOTE]
>
>AEM Forms已弃用Flex Workspace。 它可用于AEM Forms版本。
...
<br /></p> </td>
  ...
  <tr>
   ...
   <td><p>无。 <br />Workbench随附的Designer版本为仅英文版。 <br />Workbench安装程序未重新安装新版本的Designer。 而是修补与Workbench安装程序捆绑在一起的更新版本。 这也允许您在Workbench中使用本地化版本的Designer。<br /> </p> </td>
  </tr>
 </tbody>
</table>
...
在安装Workbench之前，必须确保环境包含运行它所需的软件和硬件（请参阅部分：**安装之前**）。
...
1. 以下项目现已安装在您的计算机上：
   ...
   * **Designer**：您可以从Workbench内部访问Designer。 有关信息，请参阅<a href="https://helpx.adobe.com/content/dam/help/en/experience-manager/6-5/forms/pdf/using-designer.pdf">Designer帮助</a>中的入门主题。
...
要通过HTTPS将Workbench连接到AEM Forms服务器，您必须确保将颁发公钥的证书颁发机构(CA)识别为Workbench所信任。 如果未将证书识别为来自可信来源，则必须更新[Workbench_HOME]/workbench/jre/lib/security目录中的cacert文件。
...
* **模板UUID**： Designer在每个模板中为表单及其版本插入一个唯一标识符(UUID)。 每次更新表单时，都会更新嵌入的UUID。 例如，XDP模板可能显示以下内容：

  `<?xml version="1.0" encoding="UTF-8"?>`
  `<?xfa generator="AdobeAEM formsDesignerES_V8.2" APIVersion="2.6.7185.0"?><xdp:xdp xmlns:xdp=https://ns.adobe.com/xdp/ timeStamp="2008-07-29T21:22:12Z" uuid="823e538f-ff6c-4961-b759-f7626978a223"><template xmlns="https://www.xfa.org/schema/xfa-template/2.6/">`

* **渲染选项**：在渲染的表单缓存中，缓存内容针对每组唯一渲染选项单独存储。


Forms服务通过引用文件名或存储库位置或按值接收模板，这些模板在内存中为XML对象。

* **引用传递的模板**：使用内容根和表单名称。 如果使用此方法在每个请求中传递了具有不同文件名的唯一模板，则磁盘缓存会无限增加，永远不会重复使用。 为防止出现这种情况，应使用相同的文件名传递唯一的模板，以确保为所有请求更新相同的缓存。
* **通过值**&#x200B;传递的模板：使用inDataDoc参数使用与数据一起传递的模板字节。 如果使用此方法传递具有不同UUID的唯一模板，则磁盘缓存会无限增长，并且永远不会重复使用。 要防止出现这种情况，应从所有模板中剥离UUID属性，以确保不为该模板创建缓存。 或者，传递相同的非空UUID可以创建缓存对象，但会确保随着每个请求更新相同缓存。
...
1. 在AEM Forms服务器上，在[AEM Forms根目录]\sdk\samples\Output\IVS中找到XDC文件夹。
