...
* **从发布AEM实例启动的工作流**：在从AEM发布实例提交自适应表单、交互式通信或信件时，所有工作流实例都使用服务用户创建。 在这些情况下，工作流实例数据中不会捕获登录用户的用户名。
...
1. 请按照 [访问用户数据](/help/forms/using/forms-workflow-osgi-handling-user-data.md#access) 并注意以下事项：
