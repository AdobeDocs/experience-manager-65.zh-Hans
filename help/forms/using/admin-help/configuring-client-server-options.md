...
Document Security可以跟踪可能在受保护文档上执行的各种自定义事件。 您可以在全局级别或策略级别启用Document Security Server中的事件跟踪。 然后，您可以设置JavaScript以捕获在受保护的PDF文档中执行的特定操作，如单击按钮或保存文档。 此使用情况数据将作为XML文件以键值对的形式发送，您可以将其用于进一步分析。 访问受保护文档的最终用户可以允许或拒绝来自客户端应用程序的此类跟踪。
...
**仅显示受信任的凭据颁发者：**&#x200B;选中此选项后，客户端应用程序将仅向用户提供AEM表单配置为受信任的凭据颁发者的证书（请参阅管理证书和凭据）。 如果未选择此选项，则会向用户显示用户系统上所有证书的列表。
...
1. 如果您为富水印选项选择了PDF，请单击“选择水印”PDF旁边的&#x200B;**浏览**&#x200B;以选择要用作水印的PDF文档。
...
适用于Microsoft Office的&#x200B;**Acrobat Reader DC扩展**
