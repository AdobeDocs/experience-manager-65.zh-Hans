...
A *策略* 定义了一组机密性设置和用户，用户可以访问应用了该策略的文档。 A *策略集* 用于对具有共同业务目的的一组策略进行分组。 然后，这些策略集将提供给系统中的部分用户。 有关策略的详细信息，请参阅 [策略和受策略保护的文档](/help/forms/using/admin-help/document-security.md#policies-and-policy-protected-documents).
...
首次安装Document Security时，它包含一个名为的共享策略 *限制到所有主体*. 当将此策略应用于文档时，任何可以登录Document Security的用户都可以访问该文档。 此策略位于指定的策略集中 *全局策略集*. 默认情况下，不启用此策略。 如果它适合您组织的需要，则可以启用它。
...
**策略集协调器：** 策略集协调员可以在被指定为协调员的策略集中创建和管理共享策略。 策略集协调员通常是组织内的专家，可以最好地编写特定策略集中的策略。
...
**添加用户或组：** 要添加用户或用户组，请单击“添加用户或组”，然后单击“高级搜索”以查找用户或组。 用户包括贵组织的内部用户和已向Document Security注册的受邀用户。 选择此选项时，将显示“添加用户或组”页：
...
**邀请新用户：** 要添加受邀用户，请单击“邀请新用户”，在出现的框中键入该用户的电子邮件地址，然后单击“邀请”。 仅当管理员启用此选项时，它才可用。 将新的受邀用户添加到策略时，如果尚未邀请用户注册，则Document Security会发送注册邀请电子邮件。 用户必须使用电子邮件中的链接创建帐户，然后必须激活该帐户。
...
**协作：** 允许用户使用Adobe Acrobat中的协作选项与他人协作。 此权限允许用户复制表单数据，即使策略中未明确提供复制权限。
...
**屏幕Reader：** 如果您选中“显示PDF的自定义权限”复选框，则会显示此权限。 选择此选项时，Adobe Acrobat有权向PDF添加临时标记，以提高其通过屏幕阅读器可读性。

这些文档权限在“选项”选项卡上提供。 可将这些权限应用于PDF、PTC Pro/E和Microsoft Office文件：

**脱机：** 允许用户脱机查看受此策略保护的文档。

**权限有效性：** 选择“权限始终有效”或设置文档权限有效期。 如果选择有效期，请单击日历图标以选择日期，并使用箭头指定24小时制的时间。
...
**扩展的使用跟踪：** 启用或禁用“扩展使用情况跟踪”。 Document Security支持跟踪与对PDF文件执行的各种操作相关联的用户事件。 可以使用Java脚本访问Document Security对象。 单击按钮、播放多媒体文件或保存文件是从受策略保护的PDF触发的事件的一些示例。 使用document security对象，您还可以检索用户信息。 可以从全局级别或策略级别的Document Security Server启用事件跟踪。

**自动离线租赁期：** 收件人可以离线使用受策略保护文档的最大天数（没有活动的Internet或网络连接）。 租赁期到期后，收件人必须再次同步文档才能继续使用它。
...
**允许用户名密码身份验证：** 如果要使客户端应用程序在连接到服务器时使用用户名/密码验证，请选择此选项。

**允许Kerberos身份验证：** 如果要使客户端应用程序在连接到服务器时使用Kerberos身份验证，请选择此选项。

**允许客户端证书身份验证：** 如果要使客户端应用程序在连接到服务器时使用证书身份验证，请选择此选项。
...
**仅使用经过认证的Acrobat插件进行PDF：** 如果为策略选择了此选项，则此选项指定在打开受策略保护的文档时，Acrobat 8.0及更高版本必须以认证模式运行。 当Acrobat以认证模式运行时，不会加载任何第三方插件。
...
**访问被拒绝错误消息：** 向未经许可尝试打开受策略保护文档的任何人显示的消息。 此消息会显示在Acrobat中。 无法显示此消息的客户端会显示默认消息，指示访问被拒绝。
...
* AES 128位
* AES 256位。 只有Acrobat 9.0及更高版本支持此选项。 要对PDF文件使用AES 256加密，请获取并安装Java Cryptography Extension (JCE)无限强度管辖策略文件。 这些文件将替换 [JAVE_HOME]/lib/security文件夹。 例如，如果您使用的是Sun JDK 1.6，请将下载的文件复制到 [深层根]/Java/jdk1.6.0_26/lib/security文件夹。 您可以从以下位置下载这些文件 [Java SE下载](https://java.sun.com/javase/downloads/index.jsp).
...
* 整个文档，包括其附件和元数据。 *元数据* 是有关文档及其内容的信息，您可以通过文档属性对话框或Acrobat高级菜单查看这些信息。 在Acrobat中，您可以将不同类型的文件（例如，文本、音频和视频文件）附加到PDF文档。
* 文档及其附件，但不包括元数据。
