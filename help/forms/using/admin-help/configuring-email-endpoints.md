...
**文件模式：**&#x200B;提供程序接受的传入文件附件模式。 这包括具有特定扩展名(&amp;ast；.dat、&amp;ast；.xml)、特定名称（数据）以及名称和扩展名(.``[dD][aA]``&#39;port&#39;)中的复合表达式的文件。 默认值为&amp;ast；.&amp;ast；。
...
**文件模式：**&#x200B;指定提供程序接受的传入文件附件模式。 这包括具有特定扩展名(&amp;ast；.dat、&amp;ast；.xml)、特定名称（数据）或名称和扩展名(&amp;ast；.`[dD][aA]`&#39;port&#39;)中的复合表达式的文件。
