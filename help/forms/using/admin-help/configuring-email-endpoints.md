...
**文件模式：**&#x200B;提供程序接受的传入文件附件模式。 这包括具有特定扩展名(&amp;amp；ast；.dat、&amp;amp；ast；.xml)、特定名称（数据）以及名称和扩展名()中的复合表达式的文件。``[dD][aA]``&#39;端口&#39;)。 默认值为&amp;amp；ast；。&amp;amp；ast；。
...
**重复计数：**&#x200B;电子邮件终结点扫描文件夹或目录的次数。 值–1表示无限扫描。 默认值为 -1。
...
**文件模式：**&#x200B;指定提供程序接受的传入文件附件模式。 这包括具有特定扩展名(&amp;amp；ast；.dat、&amp;amp；ast；.xml)、特定名称（数据）或名称和扩展名中的复合表达式的文件(&amp;amp；ast；)。`[dD][aA]`&#39;端口&#39;)。
