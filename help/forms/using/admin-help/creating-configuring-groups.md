...
* 基于电子邮件域(如@adobe.com)启用动态组的自动创建。 启用此功能后，用户管理会为AEM Forms数据库中的每个唯一电子邮件域创建一个动态组。 使用cron表达式指定用户管理搜索AEM表单数据库以查找新电子邮件域的频率。 这些动态组已添加到DefaultDom本地域，并命名为“具有&#x200B;*`[email domain]`*&#x200B;邮件ID的所有用户”。
