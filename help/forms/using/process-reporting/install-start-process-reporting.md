...
利用Process Reporting，AEM Forms用户能够查询当前在AEM Forms实施中定义的AEM Forms流程相关信息。 但是，流程报表不会直接从AEM Forms存储库访问数据。 数据首先按计划发布到Process Reporting存储库(*通过ProcessDataPublisher和ProcessDataStorage服务* s)。 然后，使用发布到存储库的Process Reporting数据生成Process Reporting中的报告和查询。 Process Reporting作为Forms Workflow模块的一部分安装。
...
>[!NOTE]
>
>有关“清除实用程序”的提示和技巧，请参阅Adobe Developer Connection的相关文章 [清除流程和作业](/help/forms/using/admin-help/purging-process-data.md).
...
1. &#x200B;AEM
   ...
   * (对于Linux®、AIX®和Solaris™) `[JBoss root]/bin/run.conf.sh` 文件中的文件。
...
1. 停止AEM Forms Server实例。
1. 登录到WebSphere®管理控制台。 在导航树中，单击 **服务器** > **应用程序服务器** 然后，在右窗格中单击服务器名称。

1. 在“服务器基础架构”下，单击 **Java™和进程管理** > **流程定义**.

1. 在其他属性下，单击 **Java™虚拟机**.
...
1. 在更改中心下，单击 **锁定并编辑**.
1. 在“域结构”下，单击 **环境** > **服务器** 在右窗格中，单击受控服务器名称。
...
1. **记录数**
   ...
   **CRX存储路径**
   ...
   >
   >
   >如果您在ProcessDataStorage配置中更新了Root Folder选项，则必须在ReportConfiguration服务中更新CRX Storage Path位置。
...
导航到流程报表URL (https://)时&lt;server>：&lt;port>/lc/pr)时，将显示登录屏幕。
...
登录Process Reporting时， **[!UICONTROL 主页]** 屏幕显示。

### 流程报告主屏幕 {#process-reporting-home-screen}

![process-reporting-home-screen](assets/process-reporting-home-screen.png)

**进程报告树视图：** 主屏幕左侧的树视图包含“流程报告”模块的项目。

树视图由以下顶级项目组成：

**报告：** 此项目包含随Process Reporting一起提供的现成报表。

有关预定义报表的详细信息，请参阅 [正在报告的预定义报告](/help/forms/using/process-reporting/pre-defined-reports-in-process-reporting.md).

**临时查询：** 此项目包含用于执行基于过滤器的流程和任务搜索的选项。

有关特别查询的详细信息，请参阅 [进程报告中的临时查询](/help/forms/using/process-reporting/adhoc-queries-in-process-reporting.md).
...
**进程报告标题栏：** 进程报表标题栏包含一些通用选项，您可以在用户界面中使用这些选项。
...
有关数据发布服务以及如何计划此服务的详细信息，请参阅 [计划流程数据发布](/help/forms/using/process-reporting/install-start-process-reporting.md#p-schedule-process-data-publishing-p) 请参阅《进程报告快速入门》一文。

**进程报告用户：** 登录用户名显示在上次更新时间的右侧。

**Process Reporting标题栏下拉列表：** Process Reporting标题栏右角的下拉列表包含以下选项：
