...
* **带字母的列表**：选择小写的字母列表(a，b，...) 大写(A、B、...) 书信。
...
1. 您可以选择以下选项来指定列表在CCR用户界面中的行为方式：
   ...
   * **工作分配配置**：使用此选项指定可以添加到列表的最小和最大资源数。
...
1. 您可以选择以下选项来指定条件在运行时的工作方式：
   ...
   * **禁用多个结果评估\启用多个结果评估**：启用此选项（显示为“启用多个……”）时，将评估所有条件，结果将是所有真实条件的总和。 如果禁用此选项（显示为“禁用多个……”） 然后只计算被发现为true的第一个条件，并成为该条件的输出。
...
1. 选择&#x200B;**表**&#x200B;选项卡并指定布局的以下信息：
   ...
   * **的**&#x200B;配置：选择正在配置的表。如果表是静态表，则下拉列表中表名的后缀为（静态）；如果表是动态表，则下拉列表中表名的后缀为（动态）。 静态表包含固定数量的行。 静态表可以包含目标区域和字段。 这些目标区域和字段不能绑定到重复的DDE。 绑定到表单元格的数据决定了动态表的行数。
