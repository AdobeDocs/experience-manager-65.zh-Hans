...
图表或图形是数据的可视化表示形式。 它允许您将大量信息压缩为易于理解和可视化的格式，使您能够更好地可视化、解释和分析复杂数据。
AEM Forms附加组件包提供了一个现成的Chart组件。 您可以在自适应表单和文档中使用来在可重复的面板和表中直观地表示二维数据。 图表组件允许您添加和配置以下类型的图表：
...
要配置图表，请右键单击图表组件，然后单击编辑以打开“编辑图表”对话框。 该对话框包括“标题”和“文本”、“配置”、“高级选项”和“样式”选项卡，允许您配置图表。
...
* **重复数据源的行名称**：指定作为数据来源的表行或可重复面板的元素名称。 在此示例中， statementDetails是“语句详细信息”表中可重复行的元素名称。
...
* **点颜色**：可用于点、线和点图表，以指定图表中点的颜色的十六进制值。
...
本示例中使用的信用卡对帐单是自适应文档，对帐单详细信息部分是一个表，该表在创作模式下如下所示。
...
* 行的元素名称为statementDetails。 您可以在表行属性中对其进行配置。
...
您可以配置图表以使用统计函数计算源数据的值，以便在图表上绘图。 虽然图表组件附带一些内置函数，但您可以编写自己的函数，并在图表配置中使用它们。
...
除了在图表中使用默认函数之外，您还可以在JavaScript中编写自定义函数，并在图表组件的函数列表中提供它们。
