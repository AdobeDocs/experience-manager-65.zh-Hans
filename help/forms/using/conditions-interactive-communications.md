...
1. 指定以下信息：
   ...
   * **[!UICONTROL 标记]**：（可选）要创建自定义标记，请在文本字段中输入值，然后选择Enter。 保存此条件时，将创建新添加的标记。
...
1. 选择要包含在条件中的相应资源，然后选择 **[!UICONTROL 完成]**.
   ...
   **[D] 显示预览。** 选择此图标可在“创建\编辑条件”页中显示资源的预览。
   **“server”重新排序。** 选择并按住此图标可拖放资源以在条件中对它们重新排序。
...
1. 选择 **[!UICONTROL 创建规则]** 添加规则以显示或隐藏资产。 要在规则中使用变量，请参阅 [创建变量](#variables). 有关更多信息，请参阅 [将规则添加到条件](#ruleeditor).
...
1. 创建或编辑条件时，选择 ![ruleeditoricon](assets/ruleeditoricon.png) （规则编辑器）图标。
...
1. 插入求值表达式、字符串、数据模型属性、变量或日期。
   ...
   * 创建或编辑规则时，您还可以选择 ![icon_reze](assets/icon_resize.png) （调整大小）以展开创建规则/编辑规则对话框。 展开的全窗口对话框允许您创建 [变量](#variables) 以构建规则。 再次选择调整大小以返回常规的“创建规则”对话框。
...
在条件中创建或编辑规则时，您可以选择 ![icon_reze](assets/icon_resize.png) （调整大小）展开“创建规则\编辑规则”对话框。 通过展开的全窗口对话框，您可以：
...
1. 在条件中创建或编辑规则时，您可以选择 ![icon_reze](assets/icon_resize.png) （调整大小）展开“创建规则\编辑规则”对话框。
