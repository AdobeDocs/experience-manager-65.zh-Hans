...
* 创建规则时，一个典型的经验法则是考虑您所编写规则的对象的上下文。 假定您要根据用户在字段A中指定的值隐藏或显示字段B。在这种情况下，您要评估字段A的条件，并根据它返回的值，触发字段B的操作。
  ...
  因此，如果您正在字段 B 上写入规则（要计算条件的对象），请使用条件-操作构造或规则类型。 同样，使用操作条件构造或显示或隐藏字段 A 中的规则类型。

* 有时，您需要根据一个条件执行多个操作。 在这种情况下，建议使用条件-操作构造。 在此结构中，您可以计算一次条件并指定多个操作语句。
...
* 要根据多个条件触发操作，建议使用action-condition构造。 例如，要通过评估字段 B、C 和 D 上的条件来显示和隐藏字段 A，请使用显示或隐藏字段 A 上的规则类型。
...
函数选项卡包含一组内置函数，例如，总计、最大值、平均值、数量和验证表单。 您可以使用这些函数计算可重复的面板和表格行中的值，并在编写规则时在操作和条件语句中使用它们。 但是，您也可以创建 [ 自定义函数 ](#custom-functions) 。
...
此 **[!UICONTROL 完成]** 按钮用于保存规则。 您可以保存不完整的规则。 但是，不完整内容无效，因此不会执行。 当您下次从同一表单对象启动规则编辑器时，会列出表单对象中已保存的规则。 您可以在该视图中管理现有规则。 有关更多信息，请参阅 [管理规则](#manage-rules).
...
* **重新排序**：您创建的任何新规则都会栈叠在规则列表的底部。 规则将从上到下执行。 顶部的规则先执行，然后是相同类型的其他规则。 例如，如果您分别从顶部开始，在第一、第二、第三和第四个位置执行When、Show、Enable和When规则，则顶部的When规则将首先执行，然后在第四个位置执行When规则。 然后，将执行显示和启用规则。
...
* 无线按钮， **您是现有的 Geometrixx 客户吗？**，它提供了 &quot;是&quot; 和 &quot;无&quot; 选项。 &quot;是&quot; 的值为 **0** ，而 &quot;否&quot; 为 **1** 。

* 一个文本字段， **Geometrixx 客户 id** ，用于指定客户 id。

当您在无线电上规则按钮实施此行为时，规则会在 visual 规则编辑者中显示如下。  ![规则-示例](assets/when-rule-example.png)

规则在 visual 编辑者中

在示例规则中，&quot;时间&quot; 部分中的语句是条件，当返回 True 时，将执行 &quot;Then&quot; 部分中指定的操作。

规则在代码编辑者中显示如下。

![规则-示例代码](assets/when-rule-example-code.png)
