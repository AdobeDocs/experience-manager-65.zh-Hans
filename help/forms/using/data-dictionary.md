...
1. 在Properties屏幕中，添加以下内容：
   ...
   * **扩展属性**：（可选）选择 **添加字段** 用于指定数据字典的元数据属性。 在属性名称列中，输入唯一的属性名称。 在值列中，输入要与属性关联的值。

   ![德语指定的数据字典属性](do-not-localize/1_ddproperties.png)

1. （可选）要上传数据字典的XSD架构定义，请在“数据字典结构”窗格下选择 **上载XML架构**. 浏览到XSD文件，选择它，然后选择 **打开**. 将根据上载的XML架构创建数据字典。 您需要调整数据字典中元素的显示名称和描述。 要实现此目的，请通过点按元素来选择元素的名称，并在右侧窗格的字段中编辑其描述、显示名称和其他详细信息。
...
1. （可选）选择元素后，您可以在高级选项卡中添加属性（属性）。 您还可以选择 **添加字段** 和扩展DD元素的属性。
...
1. 在数据字典页面上，选择 **选择**.
1. 选择要为其下载测试数据的数据字典，然后选择 **下载示例XML数据**.
1. 选择 **确定** 在警报消息中。 下载XML文件。
...
1. 创建数据条目后，您可以在预览包含测试数据的信件时使用此XML文件。
   ...
   您可以使用DD（选择DD并选择上载测试数据并上载此xml文件）来添加此测试数据。因此，如果正常预览信件（非自定义），则此XML数据将在信件中使用。 您还可以选择自定义，然后上载此XML。
...
1. 在“数据字典”页面中，选择 **选择** 然后选择一个数据字典以将其选定。
1. 选择 **下载示例XML数据**.
1. 选择 **确定** 在警报消息中。
...
1. 在数据字典页面上，选择 **选择** 然后选择一个数据字典以将其选定。
1. 选择 **下载本地化数据**.
1. 选择 **确定** 在警报中。 通信管理会将名为DataDictionary的zip文件下载到您的计算机 — &lt;ddname>.zip.
