...
* 要将表单数据模型属性插入到文本中，请将光标置于要插入该属性的位置，然后选择 **[A]** 属性，点按该属性，然后选择 **[!UICONTROL [B] 添加选定项]**. 也可以只双击属性以将其插入到 **[C]** 光标位置。 表单数据模型属性以棕色背景颜色突出显示。
...
* 允许代理在代理UI中编辑表单数据模型属性的值，同时 [准备和发送交互式通信](/help/forms/using/prepare-send-interactive-communication.md) 使用代理UI，选择 **[D]** 锁定该属性的图标，并确保其处于解锁状态。 属性的默认状态为锁定，代理无法在代理UI中编辑属性。
...
1. 要在文本中插入变量，请将光标放在适当的位置，选择变量，然后选择 **[!UICONTROL 添加选定项]**.
...
1. 插入求值表达式、值、数据模型属性或变量。
   ...
   * 创建或编辑规则时，您还可以选择 ![icon_reze](assets/icon_resize.png) （调整大小）以展开创建规则/编辑规则对话框。 展开的完整窗口对话框允许您拖放表单数据模型属性和变量来构建规则。 再次选择调整大小以返回创建规则对话框。
   * 您还可以在规则中创建多个条件。
...
1. 选择 **[!UICONTROL 查找并替换]**.

1. 输入要搜索的文本 **[!UICONTROL 查找]** 文本框和中的新文本（替换文本） **[!UICONTROL 替换]** 文本框并选择 **[!UICONTROL 替换]**.

1. 如果找到搜索到的文本，该文本将被替换文本替换。

   * 如果找到搜索文本的其他实例，则该实例会在文本文档片段中突出显示。 如果您选择 **[!UICONTROL 替换]** 同样，如果找到第三个实例，则高亮显示的实例会被替换，并且光标向前移动。
