...
![已缓存可用于其他组件的字体样式](assets/font-style-cached.png)
...
通过主题编辑器，可以编辑您创建或上传的主题。 导航到&#x200B;**Forms &amp; Documents > Themes**，然后选择主题并将其打开。 主题将在主题编辑器中打开。
...
* ![字段级](assets/field-level.png)：允许您切换到用于样式设置的通用组件。 如果选择数字框并选择此图标，则字段组件处于选中状态。 如果选择字段组件并选择此图标，则面板将被选中。 如果不断点击此图标进行选择，则最终将选择用于样式设置的布局。
...
在侧栏中使用&#x200B;**Dimension和位置**&#x200B;选项自定义组件的大小。 确保状态为&#x200B;**默认值**。
...
现在，您可以在自适应表单或交互式通信中使用Web服务中提供的字体。 开箱即用的AdobeWeb字体服务[Adobe Fonts](https://fonts.adobe.com/)可作为配置使用。 要使用Adobe Fonts，请创建一个套件，在其中添加字体，然后从[Adobe Fonts](https://fonts.adobe.com/)中获取套件ID。
...
1. 在&#x200B;**Cloud Service**&#x200B;页面上，导航到并打开&#x200B;**Adobe Fonts**&#x200B;选项。 打开配置文件夹，然后单击&#x200B;**创建**。
