...
* 此 `GetLoanForm` servlet呈现贷款表单。 此表单包含脚本、交互式字段、计算按钮和提交按钮。
* 用户在表单的字段中输入值并单击计算按钮。 该表单将发送至 `CalculateData` 执行脚本的Java Servlet。 该表单将发送回用户，并在表单中显示计算结果。
...
<table>
 ...
 <tbody>
  <tr>
   <td><p>1</p></td>
   <td><p>此 <code>GetLoanForm</code> 从HTML起始页调用Java Servlet。 </p></td>
  </tr>
  <tr>
   <td><p>2</p></td>
   <td><p>此 <code>GetLoanForm</code> Java Servlet使用Forms服务客户端API向客户端Web浏览器呈现贷款表单。 渲染包含配置为在服务器上运行的脚本的表单与渲染不包含脚本的表单之间的区别在于，您必须指定用于执行脚本的目标位置。 如果未指定目标位置，则不会执行配置为在服务器上运行的脚本。 例如，请考虑本节中介绍的应用程序。 此 <code>CalculateData</code> Java Servlet是执行脚本的目标位置。</p></td>
  </tr>
  <tr>
   <td><p>3</p></td>
   <td><p>用户将数据输入到交互式字段中，然后单击“计算”按钮。 该表单将发送至 <code>CalculateData</code> Java Servlet，脚本的执行位置。 </p></td>
  </tr>
  <tr>
   <td><p>4</p></td>
  ...
  <tr>
   <td><p>5</p></td>
   <td><p>当值满意时，用户单击“提交”按钮。 该表单将发送到另一个名为的Java Servlet <code>ProcessForm</code>.</p></td>
  </tr>
 </tbody>
</table>
...
验证与已提交表单关联的处理状态后， `1`，则必须将结果写回客户端Web浏览器。 显示表单时，计算值将显示在相应的字段中。
...
1. 检索包含计算脚本的表单
   ...
   * 调用 `FormsServiceClient` 对象的 `processFormSubmission` 方法并传递以下值：
      ...
      * 一个字符串值，它指定包含所有相关HTTP标头的环境变量。 通过为指定一个或多个值来指定要处理的内容类型 `CONTENT_TYPE` 环境变量。 例如，要处理XML和PDF数据，请为此参数指定以下字符串值： `CONTENT_TYPE=application/xml&CONTENT_TYPE=application/pdf`
...
1. 检索包含计算脚本的表单
   ...
   * 调用 `FormsServiceClient` 对象的 `processFormSubmission` 方法并传递以下值：
     ...
     此 `processFormSubmission` 方法填充 `FormsResultHolder` 带有表单提交结果的参数。 此 `processFormSubmission` 方法返回 `FormsResult` 包含表单提交结果的对象。
