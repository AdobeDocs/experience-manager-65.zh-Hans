---
title: 用户管理器 Java API 快速入门（SOAP）
...
下面的代码示例将Services用户角色分配给承担者，打印承担者具有的角色，然后将该角色从承担者中删除。 已为此快速启动调用两个服务： DirectoryManager服务和AuthorizationManager服务。（请参阅[管理角色和权限](/help/forms/developing/users.md#managing-roles-and-permissions)。）
