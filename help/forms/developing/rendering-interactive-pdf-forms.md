...
<table>
 ...
 <tbody>
  ...
  <tr>
   <td><p>2</p></td>
   <td><p>此 <code>GetLoanForm</code> Java Servlet使用Forms服务客户端API向客户端Web浏览器呈现贷款表单。 (请参阅 <a href="#render-an-interactive-pdf-form-using-the-java-api">使用Java API呈现交互式PDF表单</a>.)</p></td>
  </tr>
  <tr>
   <td><p>3</p></td>
  ...
  <tr>
   <td><p>5</p></td>
   <td><p>确认表单将呈现回Web浏览器。 诸如用户名字和姓氏的数据在呈现之前将与表单合并。 (请参阅 <a href="/help/forms/developing/prepopulating-forms-flowable-layouts.md">使用可流布局预填充Forms</a>.)</p></td>
  </tr>
 </tbody>
</table>
...
此互动贷款表格由范例贷款申请表提供，其附属 `GetLoanForm` Java Servlet。
...
通常，您不会将Forms服务客户端API代码放在Java Servlet的 `doGet` 或 `doPost` 方法。 最好将此代码放在单独的类中，从实例化类 `doPost` 方法(或 `doGet` 方法)，并调用适当的方法。 但是，为了代码简洁，本节中的代码示例尽量少，代码示例放在 `doPost` 方法。
...
您必须先创建Forms客户端API对象，然后才能以编程方式执行Forms服务客户端API操作。 如果您使用的是Java API，请创建 `FormsServiceClient` 对象。 如果您使用的是Forms Web服务API，请创建 `FormsService` 对象。

**指定URI值**

您可以指定Forms服务呈现表单所需的URI值。 可以使用内容根URI值引用另存为Forms应用程序一部分的表单设计 `repository:///`. 例如，考虑以下名为的表单设计 *Loan.xdp* 位于名为的Forms应用程序中 *表单应用程序*：
...
>[!NOTE]
>
>您还可以传递URL值来引用XDP文件，而不是 `com.adobe.idp.Document` Forms服务的实例。 此 `com.adobe.idp.Document` 实例包含窗体设计。 (请参阅 [将文档传递到Forms服务](/help/forms/developing/passing-documents-forms-service.md).)
...
此 `renderPDFForm` 使用 `URLSpec` 对象。 使用，将XDP文件的内容根传递到Forms服务 `URLSpec` 对象的 `setContentRootURI` 方法。 窗体设计名称( `formQuery`)作为单独的参数值传递。 将这两个值连接起来以获得对窗体设计的绝对引用。

此 `renderPDFForm2` 方法接受 `com.adobe.idp.Document` 包含要呈现的XDP或PDF文档的实例。
